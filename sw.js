...
const CACHE_NAME = 'italy-erp-v20260910427001';
...
self.addEventListener('fetch', function(event) {
  ...
  if (url.includes('/api/')) {
    var apiReq = event.request;
    try {
      apiReq = new Request(event.request, { cache: 'no-store' });
    } catch (_) {}
    event.respondWith(
      fetch(apiReq, { cache: 'no-store' }).catch(function(e) {
