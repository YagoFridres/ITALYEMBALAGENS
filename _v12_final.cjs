...
const OUT = '_V12_RESULT.jsonl';
const TARGET_VER = '20260812172500';
...
function pollVersao(target) {
  return new Promise(async (resolve) => {
    const tm = Date.now() + 15 * 60 * 1000;
    let tent = 0;
    while (Date.now() < tm) {
      tent += 1;
      const idx = await R('GET', '/index.html'); const sw = await R('GET', '/sw.js');
      const idxOk = String(idx.raw0 || '').includes(target);
      const swOk = String(sw.raw0 || '').includes(target);
      if (idxOk && swOk) { L({ t: 'DEPLOY_OK', tent, target, idxOk, swOk }); return resolve(true); }
      L({ t: 'POLL', tent, idxOk, swOk, idxVer: /v=(\d{14})/g.exec(String(idx.raw0 || ''))?.[1] || '', swVer: /v(\d{14})/g.exec(String(sw.raw0 || ''))?.[1] || '' });
      await S(10000);
    }
    resolve(false);
  });
...
function makeOf(ov = {}) {
  return Object.assign({ clinome: 'MOVEIS RIPKE', descricao: 'ZZZ_TESTE_APAGAR_V12', qtd: 10, ent: '2026-12-31', preco: 50, vendedor: 'VENDEDOR PADRAO', vendedor_id: VEND_UUID, itens: [] }, ov);
}
(async () => {
  try {
    const okD = await pollVersao(TARGET_VER);
    L({ t: 'DEPLOY_STATUS', ok: okD });
    if (!okD) process.exit(2);
    ...
    L({ t: 'ONESHOT_ITALY', total_analisado: osi.j?.total_analisado, total_alvos: osi.j?.total_alvos, atualizados: osi.j?.atualizados, detalhes10: (osi.j?.detalhes || []).slice(0, 10) });
    const oso = await R('POST', '/api/_oneshot_limpar_ofs_orfas_100k', {});
    L({ t: 'ONESHOT_ORFAS', total_analisado: oso.j?.total_analisado, total_alvos: oso.j?.total_alvos, atualizados: oso.j?.atualizados });
    await S(5000);
    ...
    if (!pn.j || parseInt(String(pn.j.proximo || '0')) !== 2605) {
      L({ t: 'PROXN_ANTES_FALHOU', esperado: 2605, recebido: pn.j?.proximo, abortar: true });
      process.exit(3);
    }
    const idsD = [];
    const casos = [];
    ...
    await runCaso('C1_BASELINE_UUID', makeOf(Object.assign({ cli_id: RIPKE_UUID_REAL, descricao: 'ZZZ_TESTE_APAGAR_V12_C1_UUID' }, baseEmp)), true);
    const p2 = makeOf(Object.assign({ cli_id: 'MOVEIS RIPKE', descricao: 'ZZZ_TESTE_APAGAR_V12_C2_EXATO' }, baseEmp)); delete p2.clinome;
    await runCaso('C2_EXATO_NOME', p2, true);
    const p3 = makeOf(Object.assign({ cli_id: 'MÓVEIS RIPKE', descricao: 'ZZZ_TESTE_APAGAR_V12_C3_ACENTO' }, baseEmp)); delete p3.clinome;
    await runCaso('C3_COM_ACENTO', p3, true);
    const p4 = makeOf(Object.assign({ cli_id: 'moveis ripke', descricao: 'ZZZ_TESTE_APAGAR_V12_C4_MIN' }, baseEmp)); delete p4.clinome;
    await runCaso('C4_MIN_SEMACENTO', p4, true);
    const p5 = makeOf(Object.assign({ cli_id: 'moveis', descricao: 'ZZZ_TESTE_APAGAR_V12_C5_PARCIAL' }, baseEmp)); delete p5.clinome;
    await runCaso('C5_PARCIAL_AMBIGUO', p5, false);
    const p6 = makeOf(Object.assign({ cli_id: 'xablau cliente 9999 nao existe zzz', descricao: 'ZZZ_TESTE_APAGAR_V12_C6_INEX' }, baseEmp)); delete p6.clinome;
