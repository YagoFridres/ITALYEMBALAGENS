...
function makeOf(ov = {}) {
  return Object.assign({ clinome: 'MOVEIS RIPKE', descricao: 'ZZZ_TESTE_APAGAR_V13', qtd: 10, ent: '2026-12-31', preco: 50, vendedor: 'VENDEDOR PADRAO', vendedor_id: VEND_UUID, itens: [] }, ov);
}
(async () => {
  try {
    ...
    await runCaso('C1_BASELINE_UUID', makeOf(Object.assign({ cli_id: RIPKE_UUID_REAL, descricao: 'ZZZ_TESTE_APAGAR_V13_C1_UUID' }, baseEmp)), true);
    const p2 = makeOf(Object.assign({ cli_id: 'MOVEIS RIPKE', descricao: 'ZZZ_TESTE_APAGAR_V13_C2_EXATO' }, baseEmp)); delete p2.clinome;
    await runCaso('C2_EXATO_NOME', p2, true);
    const p3 = makeOf(Object.assign({ cli_id: 'MÓVEIS RIPKE', descricao: 'ZZZ_TESTE_APAGAR_V13_C3_ACENTO' }, baseEmp)); delete p3.clinome;
    await runCaso('C3_COM_ACENTO', p3, true);
    const p4 = makeOf(Object.assign({ cli_id: 'moveis ripke', descricao: 'ZZZ_TESTE_APAGAR_V13_C4_MIN' }, baseEmp)); delete p4.clinome;
    await runCaso('C4_MIN_SEMACENTO', p4, true);
    const p5 = makeOf(Object.assign({ cli_id: 'moveis', descricao: 'ZZZ_TESTE_APAGAR_V13_C5_PARCIAL' }, baseEmp)); delete p5.clinome;
    await runCaso('C5_PARCIAL_AMBIGUO', p5, false);
    const p6 = makeOf(Object.assign({ cli_id: 'xablau cliente 9999 nao existe zzz', descricao: 'ZZZ_TESTE_APAGAR_V13_C6_INEX' }, baseEmp)); delete p6.clinome;
    ...
    L({ t: 'ONESHOT_ITALY_FINAL', atualizados: osi2.j?.atualizados, detalhes5: (osi2.j?.detalhes || []).slice(0, 5) });
