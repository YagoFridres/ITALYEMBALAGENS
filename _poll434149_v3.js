...
const BUMP = '20260921160000';
const EXPECTED_SHA_PREFIX = '49e8ab1';
  ...
  for (let i = 1; i <= 40; i++) {
    const result = await new Promise((resolve) => {
      ...
      const req = https.get(url, {
        headers: { 'User-Agent': 'poll-v3-434149/1.0', 'Accept': 'application/json' }
      }, (res) => {
        let b = '';
        res.on('data', (c) => { b += c; });
        res.on('end', () => {
    ...
    if (allOk) {
      console.log('\n========================================');
      console.log('✅ RAILWAY MATCHED 434149 — 3 gates OK:');
