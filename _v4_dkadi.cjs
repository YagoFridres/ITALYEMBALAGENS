...
const q='DKADI',esp=23;
const p='/api/clientes?q='+encodeURIComponent(q)+'&order=dkadi1&dir=asc';
...
const rq=https.request(o,r=>{let b='';r.on('data',d=>b+=d);r.on('end',()=>{
  if(r.statusCode!==200){process.stdout.write('DKADI HTTP'+r.statusCode+' '+b.slice(0,120)+'\n');process.exit(1);}
  try{const d=JSON.parse(b);const arr=Array.isArray(d?.data)?d.data:(Array.isArray(d)?d:[]);const c=arr.find(x=>String(x?.nome||x?.rs||'').toUpperCase().includes(q));const v=c?.total_ofs==null?null:Number(c.total_ofs);process.stdout.write('DKADI|found='+!!c+'|nome='+(c?.nome||'').slice(0,30)+'|total_ofs='+v+'|esperado='+esp+'|match='+(v===esp)+'|tempo_ms='+(Date.now()-s)+'\n');process.exit(v===esp?0:2);}
  catch(e){process.stdout.write('DKADI PARSE_ERR '+e.message+' '+b.slice(0,200)+'\n');process.exit(3);}
});});
rq.on('timeout',()=>{rq.destroy(new Error('to'));});
rq.on('error',e=>{process.stdout.write('DKADI NET '+e.message+'\n');process.exit(4);});
rq.end();
