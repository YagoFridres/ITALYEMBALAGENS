...
function makeOf(ov = {}) {
  return Object.assign({ clinome: 'MOVEIS RIPKE', descricao: 'ZZZ_TESTE_APAGAR_V15', qtd: 10, ent: '2026-12-31', preco: 50, vendedor: 'VENDEDOR PADRAO', vendedor_id: VEND_UUID, itens: [] }, ov);
}
(async () => {
  try {
    ...
    const pn2 = await R('GET', '/api/_diag_proxnum?empId=' + EMP_REAL_UUID + '&empresa_id=' + EMP_REAL_UUID);
    const MAIOR_ESP = 2527; const PROXN_ESP = '2528';
    L({ t: 'PROXN_ANTES', esperado: PROXN_ESP, recebido_prox: pn.j?.proximo, recebido_maior: pn.j?.maior, diag_resolverEmp: pn2.j?.emp_por_resolver, igual: pn2.j?.iguais, ctx_maior: pn2.j?.res_por_ctx?.maior, res_maior: pn2.j?.res_por_resolver?.maior });
    const idsD = [];
    const casos = [];
    ...
    async function runCaso(nome, payload, esperar200, verNum) {
      ...
      const numOk = (verNum != null && ok) ? String(num) === String(verNum) : true;
      const obj = { t: nome, esperava_200: esperar200, salvou: ok, passou, numOk, s: r.s };
      if (ok) {
        obj.numero = num; obj.esperava = verNum;
        obj.clinome = r.j?.data?.clinome; obj.cli = String(r.j?.data?.cli_id || '').slice(0, 20) + '...';
        if (r.j?.data?.id) idsD.push(r.j.data.id);
      } else {
        obj.err = String(r.j?.error || r.raw0 || '').slice(0, 250);
        obj.ref = r.j?.ref || null; obj.qtd = r.j?.qtd || null;
      }
      L(obj);
      casos.push({ nome, passou: passou && numOk });
    }
    let N = (pn2.j?.res_por_ctx?.maior || pn.j?.maior || 2527) + 0;
    await runCaso('C1_BASELINE_UUID', makeOf(Object.assign({ cli_id: RIPKE_UUID_REAL, descricao: 'ZZZ_TESTE_APAGAR_V15_C1_UUID' }, baseEmp)), true, N+1); N += 1;
    const p2 = makeOf(Object.assign({ cli_id: 'MOVEIS RIPKE', descricao: 'ZZZ_TESTE_APAGAR_V15_C2_EXATO' }, baseEmp)); delete p2.clinome;
    await runCaso('C2_EXATO_NOME', p2, true, N+1); N += 1;
    const p3 = makeOf(Object.assign({ cli_id: 'MÓVEIS RIPKE', descricao: 'ZZZ_TESTE_APAGAR_V15_C3_ACENTO' }, baseEmp)); delete p3.clinome;
    await runCaso('C3_COM_ACENTO', p3, true, N+1); N += 1;
    const p4 = makeOf(Object.assign({ cli_id: 'moveis ripke', descricao: 'ZZZ_TESTE_APAGAR_V15_C4_MIN' }, baseEmp)); delete p4.clinome;
    await runCaso('C4_MIN_SEMACENTO', p4, true, N+1); N += 1;
    const p5 = makeOf(Object.assign({ cli_id: 'moveis', descricao: 'ZZZ_TESTE_APAGAR_V15_C5_PARCIAL' }, baseEmp)); delete p5.clinome;
    await runCaso('C5_PARCIAL_AMBIGUO', p5, false);
    const p6 = makeOf(Object.assign({ cli_id: 'xablau cliente 9999 nao existe zzz', descricao: 'ZZZ_TESTE_APAGAR_V15_C6_INEX' }, baseEmp)); delete p6.clinome;
    ...
    L({ t: 'PROXN_FINAL', esperado: PROXN_ESP, recebido: pnF.j, ok: pnF.j && parseInt(String(pnF.j.proximo || '0')) === parseInt(PROXN_ESP, 10) });
