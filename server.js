...
app.get('/api/caixas-perdidas/dashboard', authMiddleware, async (req, res) => {
  try {
    ...
    if (!tabelaAtiva) {
      return res.json({
        _debug: 'nenhuma tabela encontrada',
        detalhamento: [],
        resumo_mes_atual: { total_caixas: 0, valor_total: 0, total_ocorrencias: 0 },
        ranking_maquinas: [],
        ranking_operadores: [],
        comparacao_mes_anterior: {}
      });
...
app.get('/api/passagens/graficos', authMiddleware, async (req, res) => {
  try {
    ...
    const empresaUuid = await _resolveEmpresaUuid(req);
