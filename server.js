...
function setNoCache(res, filePath) {
  try {
    if (filePath && String(filePath).endsWith(path.sep + 'sw.js')) {
      res.setHeader('Content-Type', 'application/javascript');
      res.setHeader('Cache-Control', 'no-cache');
      return;
    }
    if (filePath && String(filePath).endsWith(path.sep + 'manifest.json')) {
...
app.use((req, res, next) => {
  if (req.method === 'GET' && !req.path.startsWith('/api')) {
    ...
    if (p === '/sw.js') {
      ...
      res.setHeader('Content-Type', 'application/manifest+json');
      res.setHeader('Cache-Control', 'public, max-age=300');
    } else if (p === '/icon-192.png' || p === '/icon-512.png') {
...
function _pngChunk(type, data) {
  ...
  crc.writeUInt32BE(_crc32(crcBuf), 0);
  return Buffer.concat([len, t, d, crc]);
}
function _makePwaIconPng(size) {
  const s = Math.trunc(Number(size || 0) || 0);
  if (!(s === 192 || s === 512)) return null;
  if (_pwaIconCache[s]) return _pwaIconCache[s];
  const w = s, h = s;
  const stride = w * 4;
  const raw = Buffer.alloc((stride + 1) * h);
  const bg = { r: 0x0f, g: 0x11, b: 0x17 };
  const fg = { r: 0xf1, g: 0xf5, b: 0xf9 };
  const iLeft = Math.round(w * 0.46);
  const iRight = Math.round(w * 0.54);
  const iTop = Math.round(h * 0.22);
  const iBottom = Math.round(h * 0.78);
  const barW = Math.round(w * 0.22);
  const barLeft = Math.round((w - barW) / 2);
  const barRight = barLeft + barW;
  const barH = Math.round(h * 0.08);
  const topBarTop = iTop;
  const topBarBottom = iTop + barH;
  const botBarTop = iBottom - barH;
  const botBarBottom = iBottom;
  for (let y = 0; y < h; y++) {
    ...
    for (let x = 0; x < w; x++) {
      ...
      const inVert = (x >= iLeft && x <= iRight && y >= iTop && y <= iBottom);
      const inTop = (y >= topBarTop && y <= topBarBottom && x >= barLeft && x <= barRight);
      const inBot = (y >= botBarTop && y <= botBarBottom && x >= barLeft && x <= barRight);
      if (inVert || inTop || inBot) { r = fg.r; g = fg.g; b = fg.b; }
...
async function _autoSugerirMaquinaParaOF(body, created){
  try{
    ...
    const comp = Number(src?.comprimento_mm ?? src?.caixa_comprimento ?? src?.caixaComprimento ?? src?.comprimento ?? 0) || 0;
    const larg = Number(src?.largura_mm ?? src?.caixa_largura ?? src?.caixaLargura ?? src?.largura ?? 0) || 0;
    const alt  = Number(src?.altura_mm ?? src?.caixa_altura ?? src?.caixaAltura ?? src?.altura ?? 0) || 0;
    if(!(comp > 0 && larg > 0 && alt > 0)) return { ok:false, skipped:'sem_medidas' };
    ...
    console.log('[SUGESTAO MAQ]', {
      comp, larg, alt, desenvolvimento, boca,
      total_maquinas: maquinasAtivas.length,
    ...
    const filas = await Promise.all(maquinasParaFila.map(async (m) => {
      try {
        ...
        const { count, error: eCount } = await supabase
          ...
          .contains('fluxo_maquinas', [nomeMaq])
          .not('status', 'in', statusNotIn)
          .is('deleted_at', null);
        return { ...m, fila: eCount ? 999 : Math.trunc(Number(count || 0)) };
      } catch(_) {
        return { ...m, fila: 999 };
      }
    }));
    ...
    filas.sort((a, b) =>
      ...
      String(a.nome || a.col || '').localeCompare(String(b.nome || b.col || ''))
    );

    const melhor = filas[0];
    if (!melhor) return { ok: false, skipped: 'sem_melhor' };

    const nomeMelhor = String(melhor.nome || melhor.col || '').trim();
    if (!nomeMelhor) return { ok: false, skipped: 'nome_vazio' };

    const updPayload = {
      fluxo_maquinas: [nomeMelhor],
      maq: JSON.stringify([nomeMelhor]),
    ...
    return {
      ok: true,
      nome: nomeMelhor,
      updated: upd?.data || null,
      compat_count: compat.length,
      fallback_usado: compat.length === 0,
    };
  }catch(e){
    return { ok:false, skipped:'erro', error: String(e?.message || e) };
  }
...
app.get('/api/passagens/graficos', authMiddleware, async (req, res) => {
  try {
    ...
    const seteISO = new Date(seteDiasAtras.getFullYear(), seteDiasAtras.getMonth(), seteDiasAtras.getDate(), 0, 0, 0).toISOString().slice(0, 10);
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const iso = new Date(d.getFullYear(), d.getMonth(), d.getDate(), 12, 0, 0).toISOString().slice(0, 10);
      const lbl = d.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' });
      labels.push(lbl);
      labelToIdx.set(iso, labels.length - 1);
    }

    const porMaquina = {};
    MAQS.forEach((m) => { porMaquina[m] = new Array(7).fill(0); });

    const baseWeek = () => supabase
      .from('passagens_maquina')
      .select('data_passagem,maquina,hora_passagem,emp_id,empresa_id')
      .gte('data_passagem', seteISO)
      .lte('data_passagem', hojeISO)
      .order('data_passagem', { ascending: true });
    ...
    let weekRows = [];
    {
      ...
      if (r?.error) {
        ...
        if (msg.includes('does not exist') || msg.includes('Could not find') || msg.includes('column')) {
          let q2 = supabase
            .from('passagens_maquina')
            .select('data_passagem,maquina,hora_passagem')
            .gte('data_passagem', seteISO)
            .lte('data_passagem', hojeISO)
            .order('data_passagem', { ascending: true });
          const r2 = await q2;
          if (r2?.error) throw r2.error;
          weekRows = Array.isArray(r2.data) ? r2.data : [];
        } else {
          throw r.error;
        }
      } else {
        weekRows = Array.isArray(r.data) ? r.data : [];
      }
    }

    weekRows.forEach((p) => {
      const iso = String(p?.data_passagem || '').slice(0, 10);
      const idx = labelToIdx.get(iso);
      if (idx == null) return;
      const canon = _canonMaqNome(p?.maquina || '');
      if (!canon || !porMaquina[canon]) return;
      porMaquina[canon][idx] = (porMaquina[canon][idx] || 0) + 1;
    });

    const baseMonth = () => supabase
      .from('passagens_maquina')
      .select('maquina,emp_id,empresa_id,data_passagem')
      .gte('data_passagem', inicioMesISO)
      .lte('data_passagem', hojeISO);
    ...
    let mesRows = [];
    {
      ...
      if (r?.error) {
        ...
        if (msg.includes('does not exist') || msg.includes('Could not find') || msg.includes('column')) {
          let q2 = supabase
            .from('passagens_maquina')
            .select('maquina,data_passagem')
            .gte('data_passagem', inicioMesISO)
            .lte('data_passagem', hojeISO);
          const r2 = await q2;
          if (r2?.error) throw r2.error;
          mesRows = Array.isArray(r2.data) ? r2.data : [];
        } else {
          throw r.error;
        }
      } else {
        mesRows = Array.isArray(r.data) ? r.data : [];
      }
    }

    const totalMes = {};
    MAQS.forEach((m) => { totalMes[m] = 0; });
    mesRows.forEach((p) => {
      const canon = _canonMaqNome(p?.maquina || '');
      if (!canon || totalMes[canon] === undefined) return;
      totalMes[canon] = (totalMes[canon] || 0) + 1;
    });

    return res.json({ ok: true, semana: { labels, porMaquina }, mes: totalMes });
  } catch (e) { err(res, e); }
});
...
function _cnpjIs14(d) {
  return /^\d{14}$/.test(String(d || ''));
}

async function _clientesFindByCnpjDigits({ digits, empId, ignoreId }) {
  if (!_cnpjIs14(digits)) return [];
  let q = supabase
    .from('clientes')
    .select('id,nome,cnpj,emp_id,empId,empresa_id')
    .ilike('cnpj', `%${digits}%`)
    .limit(10);
  if (empId) q = q.eq('emp_id', empId);
  const { data, error } = await q;
  ...
  const ign = String(ignoreId || '').trim();
...
async function _getEstoqueMateriaisTableName() {
  if (_ESTOQUE_MATERIAIS_TABLE) return _ESTOQUE_MATERIAIS_TABLE;
  _ESTOQUE_MATERIAIS_TABLE = 'estoque_materiais';
  try {
    if (!supabase || typeof supabase.schema !== 'function') return _ESTOQUE_MATERIAIS_TABLE;
    const { data, error } = await supabase
      .schema('information_schema')
      .from('tables')
      .select('table_name')
      .eq('table_schema', 'public')
      .ilike('table_name', '%material%')
      .limit(50);
    if (error) return _ESTOQUE_MATERIAIS_TABLE;
    const names = (Array.isArray(data) ? data : []).map((r) => String(r?.table_name || '').trim()).filter(Boolean);
    const pick =
      names.find((n) => n === 'estoque_materiais') ||
      names.find((n) => n === 'estoque_material') ||
      names.find((n) => n.startsWith('estoque_') && n.includes('mater')) ||
      names[0];
    if (pick) _ESTOQUE_MATERIAIS_TABLE = pick;
  } catch (_) {}
  return _ESTOQUE_MATERIAIS_TABLE;
}

async function _getEstoqueMateriaisMovTableName() {
  if (_ESTOQUE_MATERIAIS_MOV_TABLE) return _ESTOQUE_MATERIAIS_MOV_TABLE;
  _ESTOQUE_MATERIAIS_MOV_TABLE = 'estoque_materiais_movimentos';
  try {
    if (!supabase || typeof supabase.schema !== 'function') return _ESTOQUE_MATERIAIS_MOV_TABLE;
    const { data, error } = await supabase
      .schema('information_schema')
      .from('tables')
      .select('table_name')
      .eq('table_schema', 'public')
      .ilike('table_name', '%material%mov%')
      .limit(50);
    if (error) return _ESTOQUE_MATERIAIS_MOV_TABLE;
    const names = (Array.isArray(data) ? data : []).map((r) => String(r?.table_name || '').trim()).filter(Boolean);
    const pick =
      names.find((n) => n === 'estoque_materiais_movimentos') ||
      names.find((n) => n === 'estoque_material_movimentos') ||
      names.find((n) => n.includes('mov') && n.includes('mater')) ||
      names[0];
    if (pick) _ESTOQUE_MATERIAIS_MOV_TABLE = pick;
  } catch (_) {}
  return _ESTOQUE_MATERIAIS_MOV_TABLE;
}
...
app.put('/api/estoque_materiais/:id', authMiddleware, async (req, res) => {
  try {
    ...
    Object.keys(upd).forEach((k) => upd[k] === undefined && delete upd[k]);
    const table = await _getEstoqueMateriaisTableName();
    const { data, error } = await supabase.from(table).update(upd).eq('id', req.params.id).eq('empresa_id', empresaId).select();
    if (error) throw error;
    return ok(res, data && data[0] ? data[0] : null);
  } catch (e) { err(res, e); }
});
...
app.post('/api/estoque_materiais/:id/movimentos', authMiddleware, async (req, res) => {
  try {
    ...
    const table = await _getEstoqueMateriaisTableName();
    const movTable = await _getEstoqueMateriaisMovTableName();
    const { data: cur, error: curErr } = await supabase.from(table).select('id, quantidade_atual').eq('id', req.params.id).eq('empresa_id', empresaId).maybeSingle();
    if (curErr) throw curErr;
    if (!cur) return res.status(404).json({ ok: false, error: 'Item não encontrado' });
    ...
    const { data: upd, error: updErr } = await supabase.from(table)
      .update({ quantidade_atual: nova, updated_at: now })
      .eq('id', cur.id)
      .eq('empresa_id', empresaId)
      .select();
    ...
    const { data: m1, error: mErr } = await supabase.from(movTable).insert([mov]).select();
    if (mErr) throw mErr;
    return ok(res, { item: upd && upd[0] ? upd[0] : null, movimento: m1 && m1[0] ? m1[0] : null });
  } catch (e) { err(res, e); }
});
