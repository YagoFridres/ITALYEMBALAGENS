const wantedP = '20260813080600';
const wantedC = '0355792';
        ...
        try {
          ...
          if (p === wantedP || c === wantedC) {
            ...
            try {
              ...
              const to2 = setTimeout(() => ctrl2.abort(), 120000);
              const r2 = await fetch(BASE + '/api/_diag_public_v0804?key=' + key, { signal: ctrl2.signal });
              clearTimeout(to2);
              console.log('PROBE_HTTP=' + r2.status);
              const txt = await r2.text();
              process.stdout.write(txt);
            } catch (e) { console.log('PROBE_ERR=' + String(e.message || e).slice(0, 400)); }
            process.exit(0);
          }
        } catch (_e) { console.log('json parse fail: ' + String(_e?.message||_e).slice(0,200)); }
      } else {
