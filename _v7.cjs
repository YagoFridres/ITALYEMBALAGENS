...
const RIPKE_UUID_REAL = 'be617df1-441a-4f11-918e-d813a5ac854c';  // ✅ REAL do banco!
const VEND_UUID = '00000000-0000-0000-0000-000000000001';
const OUT = '_V7_RESULT.jsonl';
...
function makeOf(ov = {}) {
  return Object.assign({ clinome: 'MOVEIS RIPKE', descricao: 'ZZZ_TESTE_APAGAR_V7', qtd: 10, ent: '2026-12-31', preco: 50, vendedor: 'VENDEDOR PADRAO', vendedor_id: VEND_UUID, itens: [] }, ov);
}
async function pollDeploy(alvo) {
  for (let i = 0; i < 35; i++) {
    const r = await R('GET', '/patch.js');
    const m = (r.raw0 || '').match(/PATCH_RUNTIME_VERSION\s*=\s*['"](\d+)['"]/i);
    const v = m && m[1] ? m[1] : null;
    if (v === alvo) return { ok: true, v };
    L({ t: 'WAIT_DEPLOY', i, v });
    await S(9000);
  }
  return { ok: false };
...
(async () => {
  try {
    const dp = await pollDeploy('20260812154000');
    L({ t: 'DEPLOY', ...dp });
    ...
    L({ t: 'ONESHOT_ITALY', s: osi.s, total_alvos: osi.j?.total_alvos, atualizados: osi.j?.atualizados, detalhes: (osi.j?.detalhes || []).slice(0, 15) });
    const oso = await R('POST', '/api/_oneshot_limpar_ofs_orfas_100k', {});
    L({ t: 'ONESHOT_ORFAS', s: oso.s, total_alvos: oso.j?.total_alvos, atualizados: oso.j?.atualizados });
    await S(4000);
    ...
    await runCaso('C1_BASELINE_UUID', makeOf(Object.assign({ cli_id: RIPKE_UUID_REAL, descricao: 'ZZZ_TESTE_APAGAR_V7_C1_UUID' }, baseEmp)), true);
    const p2 = makeOf(Object.assign({ cli_id: 'MOVEIS RIPKE', descricao: 'ZZZ_TESTE_APAGAR_V7_C2_EXATO' }, baseEmp)); delete p2.clinome;
    await runCaso('C2_EXATO_NOME', p2, true);
    const p3 = makeOf(Object.assign({ cli_id: 'MÓVEIS RIPKE', descricao: 'ZZZ_TESTE_APAGAR_V7_C3_ACENTO' }, baseEmp)); delete p3.clinome;
    await runCaso('C3_COM_ACENTO', p3, true);
    const p4 = makeOf(Object.assign({ cli_id: 'moveis ripke', descricao: 'ZZZ_TESTE_APAGAR_V7_C4_MIN' }, baseEmp)); delete p4.clinome;
    await runCaso('C4_MIN_SEMACENTO', p4, true);
    const p5 = makeOf(Object.assign({ cli_id: 'moveis', descricao: 'ZZZ_TESTE_APAGAR_V7_C5_PARCIAL' }, baseEmp)); delete p5.clinome;
    await runCaso('C5_PARCIAL_AMBIGUO', p5, false);
    const p6 = makeOf(Object.assign({ cli_id: 'xablau cliente 9999 nao existe zzz', descricao: 'ZZZ_TESTE_APAGAR_V7_C6_INEX' }, baseEmp)); delete p6.clinome;
    await runCaso('C6_INEXISTENTE', p6, false);
    await runCaso('C7_CLINOME_NORMAL', makeOf(Object.assign({ descricao: 'ZZZ_TESTE_APAGAR_V7_C7_CLINOME' }, baseEmp)), true);
    const dels2 = [];
    for (const id of idsD) { const d = await R('DELETE', '/api/ofs/' + id); dels2.push({ id: String(id).slice(0, 15), s: d.s, ok: !!d.j?.ok }); await S(250); }
