...
const OUT = '_V9_RESULT.jsonl';
const ALVO = '20260812161000';
...
function makeOf(ov = {}) {
  return Object.assign({ clinome: 'MOVEIS RIPKE', descricao: 'ZZZ_TESTE_APAGAR_V9', qtd: 10, ent: '2026-12-31', preco: 50, vendedor: 'VENDEDOR PADRAO', vendedor_id: VEND_UUID, itens: [] }, ov);
}
async function pollDeploy() {
  for (let i = 0; i < 40; i++) {
    let htmlV = null, swV = null;
    try { const h = await R('GET', '/index.html'); const m = (h.raw0 || '').match(/patch\.js\?v=(\d{14})/); if (m && m[1]) htmlV = m[1]; } catch (_) {}
    try { const s = await R('GET', '/sw.js'); const m = (s.raw0 || '').match(/italy-erp-v(\d{14})/); if (m && m[1]) swV = m[1]; } catch (_) {}
    if (htmlV === ALVO && swV === ALVO) return { ok: true, htmlV, swV };
    L({ t: 'WAIT_DEPLOY', i, htmlV, swV });
    await S(9000);
  }
  return { ok: false };
}
(async () => {
  try {
    const dp = await pollDeploy(); L({ t: 'DEPLOY', ...dp });
    ...
    L({ t: 'ONESHOT_ITALY', s: osi.s, total_analisado: osi.j?.total_analisado, total_alvos: osi.j?.total_alvos, atualizados: osi.j?.atualizados, detalhes: (osi.j?.detalhes || []).slice(0, 30) });
    await S(4000);
    const oso = await R('POST', '/api/_oneshot_limpar_ofs_orfas_100k', {});
    L({ t: 'ONESHOT_ORFAS', s: oso.s, total_analisado: oso.j?.total_analisado, total_alvos: oso.j?.total_alvos, atualizados: oso.j?.atualizados });
    await S(6000);
    const pn = await R('GET', '/api/ofs/proximo-numero?empId=' + EMP_REAL_UUID);
    L({ t: 'PROXN_2605_ANTES', ok: pn.j && parseInt(String(pn.j.proximo || '0')) === 2605, resp: pn.j });
    const idsD = [];
    const casos = [];
    ...
    async function runCaso(nome, payload, esperar200) {
      ...
      const obj = { t: nome, esperava_200, salvou: ok, passou, s: r.s };
      if (ok) {
    ...
    await runCaso('C1_BASELINE_UUID', makeOf(Object.assign({ cli_id: RIPKE_UUID_REAL, descricao: 'ZZZ_TESTE_APAGAR_V9_C1_UUID' }, baseEmp)), true);
    const p2 = makeOf(Object.assign({ cli_id: 'MOVEIS RIPKE', descricao: 'ZZZ_TESTE_APAGAR_V9_C2_EXATO' }, baseEmp)); delete p2.clinome;
    await runCaso('C2_EXATO_NOME', p2, true);
    const p3 = makeOf(Object.assign({ cli_id: 'MÓVEIS RIPKE', descricao: 'ZZZ_TESTE_APAGAR_V9_C3_ACENTO' }, baseEmp)); delete p3.clinome;
    await runCaso('C3_COM_ACENTO', p3, true);
    const p4 = makeOf(Object.assign({ cli_id: 'moveis ripke', descricao: 'ZZZ_TESTE_APAGAR_V9_C4_MIN' }, baseEmp)); delete p4.clinome;
    await runCaso('C4_MIN_SEMACENTO', p4, true);
    const p5 = makeOf(Object.assign({ cli_id: 'moveis', descricao: 'ZZZ_TESTE_APAGAR_V9_C5_PARCIAL' }, baseEmp)); delete p5.clinome;
    await runCaso('C5_PARCIAL_AMBIGUO', p5, false);
    const p6 = makeOf(Object.assign({ cli_id: 'xablau cliente 9999 nao existe zzz', descricao: 'ZZZ_TESTE_APAGAR_V9_C6_INEX' }, baseEmp)); delete p6.clinome;
    ...
    L({ t: 'ONESHOT_ITALY_POS_LIMPEZA', s: osi2.s, alvos: osi2.j?.total_alvos, atualizados: osi2.j?.atualizados });
    const oso2 = await R('POST', '/api/_oneshot_limpar_ofs_orfas_100k', {});
    L({ t: 'ONESHOT_ORFAS_POS_LIMPEZA', s: oso2.s, alvos: oso2.j?.total_alvos, atualizados: oso2.j?.atualizados });
    await S(6000);
    ...
    L({ t: 'ERR', msg: String(e), st: String(e.stack || '').slice(0, 500) });
    process.exit(1);
  }
})();
