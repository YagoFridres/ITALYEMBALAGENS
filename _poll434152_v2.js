...
function one() {
  attempts++;
  https.get('https://adm.italyembalagens.com.br/api/version', { headers: { 'User-Agent': 'poll-434152-v2' } }, (r) => {
    let d = '';
    r.on('data', c => d += c);
    r.on('end', () => {
      try {
        ...
        const m1 = p === '20260921191000';
        const m2 = b === 'main';
        const m3 = cr.indexOf('1643f53') === 0;
        ...
        if (m1 && m2 && m3) { console.log('\n✅ 3/3 GATES MATCHED Railway deploy 434152 R7-T3 Gastos por Centro concluído!'); process.exit(0); }
