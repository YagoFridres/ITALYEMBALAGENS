...
if (!window._menuPatchFinal) {
  ...
  function _injetarMenusCustom() {
    if (!window._menuDiagFeito) {
      window._menuDiagFeito = true;
      var textos = Array.from(document.querySelectorAll('li'))
        .filter(function(li) { return li.offsetParent !== null; })
        .map(function(li) { return li.textContent.trim().substring(0, 25); })
        .filter(function(t) { return t.length > 1; });
      console.log('[MENU DIAG] LIs visíveis:', textos);
    }

    var todosLiVisiveis = Array.from(document.querySelectorAll('li'))
      .filter(function(li) { return li.offsetParent !== null; });

    if (!document.getElementById('_menu_gramaturas')) {
      var ancCad = todosLiVisiveis.find(function(li) {
        var t = li.textContent.trim();
        return t === 'Clientes' || t === 'Fornecedores' ||
               t === 'Vendedores' || t === 'Usuários' || t === 'Usuarios' ||
               t === 'Mapa de Clientes';
      });
      if (ancCad && ancCad.parentElement) {
        var novoLi = document.createElement('li');
        novoLi.id = '_menu_gramaturas';
        novoLi.className = ancCad.className;
        var novoA = document.createElement('a');
        novoA.href = '#';
        novoA.textContent = '📐 Gramaturas';
        novoA.className = (ancCad.querySelector('a') || {}).className || '';
        novoA.addEventListener('click', function(e) {
          e.preventDefault(); e.stopPropagation();
          if (typeof window.renderGramaturasPage === 'function') window.renderGramaturasPage();
          else if (window._patchCustomPages && window._patchCustomPages['gramaturas']) window._patchCustomPages['gramaturas']();
        });
        novoLi.appendChild(novoA);
        ancCad.parentElement.appendChild(novoLi);
        console.log('[MENU] ✓ Gramaturas injetado');
      }
    }

    if (!document.getElementById('_menu_toneladas')) {
      var ancFin = todosLiVisiveis.find(function(li) {
        var t = li.textContent.trim();
        return t === 'Comissões' || t === 'Comissoes' ||
               t === 'Orçamentos' || t === 'Orcamentos';
      });
      if (ancFin && ancFin.parentElement) {
        var novoLi2 = document.createElement('li');
        novoLi2.id = '_menu_toneladas';
        novoLi2.className = ancFin.className;
        var novoA2 = document.createElement('a');
        novoA2.href = '#';
        novoA2.textContent = '⚖️ Toneladas Vendidas';
        novoA2.className = (ancFin.querySelector('a') || {}).className || '';
        novoA2.addEventListener('click', function(e) {
          e.preventDefault(); e.stopPropagation();
          if (typeof window.renderToneladasPage === 'function') window.renderToneladasPage();
          else if (window._patchCustomPages && window._patchCustomPages['toneladas-vendidas']) window._patchCustomPages['toneladas-vendidas']();
        });
        novoLi2.appendChild(novoA2);
        ancFin.parentElement.appendChild(novoLi2);
        console.log('[MENU] ✓ Toneladas injetado');
      }
    }

    if (!document.getElementById('_menu_caixas_fin')) {
      Array.from(document.querySelectorAll('li')).forEach(function(li) {
        if (li.id === '_menu_caixas_fin') return;
        var t = li.textContent.trim();
        if (t === 'Caixas Perdidas' && String(li.id || '').indexOf('_menu_caixas_fin') === -1) {
          li.style.display = 'none';
          li.dataset.removidoPatch = '1';
        }
      });

      var ancFin3 = todosLiVisiveis.find(function(li) {
        var t = li.textContent.trim();
        return t === 'Comissões' || t === 'Comissoes' ||
               t === 'Orçamentos' || t === 'Orcamentos';
      });
      if (ancFin3 && ancFin3.parentElement) {
        var novoLi3 = document.createElement('li');
        novoLi3.id = '_menu_caixas_fin';
        novoLi3.className = ancFin3.className;
        var novoA3 = document.createElement('a');
        novoA3.href = '#';
        novoA3.textContent = '📦 Caixas Perdidas';
        novoA3.className = (ancFin3.querySelector('a') || {}).className || '';
        novoA3.addEventListener('click', function(e) {
          e.preventDefault(); e.stopPropagation();
        ...
        novoLi3.appendChild(novoA3);
        ancFin3.parentElement.appendChild(novoLi3);
        console.log('[MENU] ✓ Caixas Perdidas injetado em Financeiro');
      }
    }

    var ok = document.getElementById('_menu_gramaturas') &&
             document.getElementById('_menu_toneladas') &&
             document.getElementById('_menu_caixas_fin');
    if (ok) {
      console.log('[MENU] ✓ todos os itens injetados');
      if (window._menuInjetarInterval) {
        clearInterval(window._menuInjetarInterval);
        window._menuInjetarInterval = null;
      }
    }
  }

  _injetarMenusCustom();
  window._menuInjetarInterval = setInterval(_injetarMenusCustom, 2000);
  setTimeout(function() {
    if (window._menuInjetarInterval) clearInterval(window._menuInjetarInterval);
  }, 60000);
}

(function _ocultarRelatorioMensal() {
  if (!document.getElementById('_hide_relatorio_mensal')) {
    var s = document.createElement('style');
    s.id = '_hide_relatorio_mensal';
    s.textContent = 'li:has(a[onclick*="relatorio-mensal"]), li:has(a[href*="relatorio-mensal"]) { display: none !important; }';
    document.head.appendChild(s);
  }
  function ocultarAgora() {
    Array.from(document.querySelectorAll('li, a')).forEach(function(el) {
      if (el.textContent.trim() === 'Relatório Mensal' ||
          el.textContent.trim() === 'Relatorio Mensal' ||
          el.textContent.trim() === 'Rel. Mensal') {
...
(function patchComissoesMenusToneladasClonePcpMobile() {
  if (window.__patchComissoesMenusToneladasClonePcpMobile) return;
  window.__patchComissoesMenusToneladasClonePcpMobile = true;

  function tokenHeaders(extra) {
    var token = '';
    try {
      token = String(window._token || localStorage.getItem('token') || localStorage.getItem('access_token') || sessionStorage.getItem('token') || '').trim();
    } catch (_) {}
  ...
  function hotfixToast(msg, tone) {
    try { if (typeof window.toast === 'function') return window.toast(String(msg || ''), tone || 'var(--accent)'); } catch (_) {}
    try { if (typeof window.mostrarToast === 'function') return window.mostrarToast(String(msg || '')); } catch (_) {}
  ...
  function fetchJsonPatch(url, opt) {
  ...
  function isConcluidaStatus(st) {
    var s = normText(st);
    return s.indexOf('conclu') >= 0 || s === 'pedido pronto' || s === 'entregue' || s === 'despachada' || s === 'finalizada';
  }

  function isCanceladaStatus(st) {
    var s = normText(st);
    return s.indexOf('cancel') >= 0 || s.indexOf('exclu') >= 0;
  }

  function isUrgenteOf(of) {
    return !!(of && (of.urgente === true || of.urg === true || String(of.urgente || of.urg || '').toLowerCase() === 'true' || String(of.urgente || of.urg || '') === '1' || normText(of && of.prioridade).indexOf('urgente') >= 0));
  }
  ...
  function normalizeComissaoRow(of, vendPctMap) {
    ...
    return {
      ...
      data: String(of.data_conclusao || of.dia || '—').trim() || '—',
      raw: of
    };
  }

  async function loadVendedoresPatch() {
    if (Array.isArray(window.__COM2_VENDEDORES) && window.__COM2_VENDEDORES.length) return window.__COM2_VENDEDORES;
    var out = await fetchJsonPatch('/api/vendedores', { headers: tokenHeaders() });
    var list = (out.json && (out.json.data || out.json.vendedores || out.json)) || [];
    window.__COM2_VENDEDORES = Array.isArray(list) ? list : [];
    return window.__COM2_VENDEDORES;
  }

  async function loadGramaturasPatch() {
    if (Array.isArray(window.__COM2_GRAMATURAS) && window.__COM2_GRAMATURAS.length) return window.__COM2_GRAMATURAS;
    var out = await fetchJsonPatch('/api/gramaturas', { headers: tokenHeaders() });
    var list = (out.json && (out.json.data || out.json)) || [];
    window.__COM2_GRAMATURAS = Array.isArray(list) ? list : [];
    return window.__COM2_GRAMATURAS;
  }

  function ensureComissoesTrocarModal() {
    if (document.getElementById('com2-modal')) return;
    var style = document.createElement('style');
    style.id = 'com2-style';
    style.textContent = ''
      + '#com2-modal{position:fixed;inset:0;z-index:100120;background:rgba(2,6,23,.75);display:none;align-items:center;justify-content:center;padding:16px}'
      + '#com2-modal .com2-box{width:min(920px,96vw);max-height:92vh;overflow:auto;background:#0f172a;border:1px solid rgba(255,255,255,.12);border-radius:16px;box-shadow:0 30px 80px rgba(0,0,0,.45);padding:18px;color:#f8fafc}'
      + '#com2-modal .com2-head{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:14px}'
      + '#com2-modal .com2-title{font-size:18px;font-weight:800}'
      + '#com2-modal .com2-close{background:transparent;border:1px solid rgba(255,255,255,.16);border-radius:10px;color:#fff;padding:8px 12px;cursor:pointer}'
      + '#com2-modal .com2-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}'
      + '#com2-modal .com2-full{grid-column:1/-1}'
      + '#com2-modal label{display:block;font-size:12px;color:#94a3b8;margin-bottom:6px}'
      + '#com2-modal input,#com2-modal select,#com2-modal textarea{width:100%;box-sizing:border-box;background:#111827;border:1px solid rgba(255,255,255,.12);border-radius:10px;padding:11px 12px;color:#fff;min-height:44px}'
      + '#com2-modal textarea{min-height:100px;resize:vertical}'
      + '#com2-modal .com2-actions{display:flex;justify-content:flex-end;gap:10px;margin-top:16px}'
      + '#com2-modal .com2-cancel,#com2-modal .com2-save{border:none;border-radius:10px;padding:11px 16px;cursor:pointer;min-height:44px}'
      + '#com2-modal .com2-cancel{background:#334155;color:#fff}'
      + '#com2-modal .com2-save{background:#16a34a;color:#fff;font-weight:700}'
      + '@media (max-width:768px){#com2-modal .com2-box{width:95vw;max-width:95vw}#com2-modal .com2-grid{grid-template-columns:1fr}}';
    document.head.appendChild(style);
    var modal = document.createElement('div');
    modal.id = 'com2-modal';
    modal.innerHTML = ''
      + '<div class="com2-box">'
      + '  <div class="com2-head"><div class="com2-title" id="com2-title">Editar OF</div><button class="com2-close" type="button" id="com2-close">Fechar</button></div>'
      + '  <div id="com2-body"></div>'
      + '</div>';
    document.body.appendChild(modal);
    modal.addEventListener('click', function(e) {
      if (e.target === modal) modal.style.display = 'none';
    });
    var closeBtn = document.getElementById('com2-close');
    if (closeBtn) closeBtn.onclick = function() { modal.style.display = 'none'; };
  }

  function com2SetTotal() {
    var qtd = toNum((document.getElementById('com2-qtd') || {}).value);
    var vu = toNum((document.getElementById('com2-vl-unit') || {}).value);
    var total = Math.round(qtd * vu * 100) / 100;
    var totalEl = document.getElementById('com2-valor-total');
    if (totalEl) totalEl.value = total.toFixed(2);
  }

  async function openComissoesTrocarModal(ofId) {
    ofId = String(ofId || '').trim();
    if (!ofId) return;
    ensureComissoesTrocarModal();
    var modal = document.getElementById('com2-modal');
    var title = document.getElementById('com2-title');
    var body = document.getElementById('com2-body');
    if (!modal || !body) return;
    body.innerHTML = '<div style="padding:16px;color:#94a3b8">Carregando OF...</div>';
    modal.style.display = 'flex';
    try {
      var ofResp = await fetchJsonPatch('/api/ofs/' + encodeURIComponent(ofId), { headers: tokenHeaders() });
      var of = (ofResp.json && (ofResp.json.data || ofResp.json)) || null;
      ...
      body.innerHTML = ''
        ...
        + '  <div><label>Quantidade</label><input type="number" step="1" id="com2-qtd" value="' + esc(qtd) + '"></div>'
        + '  <div><label>Valor Unitário</label><input type="number" step="0.01" id="com2-vl-unit" value="' + esc(vlUnit.toFixed(2)) + '"></div>'
        + '  <div><label>Valor Total</label><input type="number" step="0.01" id="com2-valor-total" readonly value="' + esc(valorTotal.toFixed(2)) + '"></div>'
        + '  <div><label>% Comissão</label><input type="number" step="0.01" id="com2-comissao-pct" value="' + esc(pct.toFixed(2)) + '"></div>'
        + '  <div><label>Gramatura</label><select id="com2-gramatura"><option value="">— Sem gramatura —</option>' + (gramaturas || []).map(function(g) {
          return '<option value="' + esc(String(g && g.id || '')) + '">' + esc(String(g && (g.nome || g.gramatura || '—') || '—')) + '</option>';
        }).join('') + '</select></div>'
        ...
        + '  <button type="button" class="com2-cancel" id="com2-cancel">Fechar</button>'
      ...
      if (qtdEl) qtdEl.oninput = com2SetTotal;
      if (vuEl) vuEl.oninput = com2SetTotal;
      var cancelBtn = document.getElementById('com2-cancel');
      if (cancelBtn) cancelBtn.onclick = function() { modal.style.display = 'none'; };
      var saveBtn = document.getElementById('com2-save');
      if (saveBtn) {
        saveBtn.onclick = async function() {
          ...
          try {
            ...
            if (!saveResp.response.ok || (saveResp.json && saveResp.json.ok === false)) {
              throw new Error((saveResp.json && (saveResp.json.error || saveResp.json.message)) || ('HTTP ' + saveResp.response.status));
            }
            modal.style.display = 'none';
            hotfixToast('OF atualizada com sucesso', 'var(--green)');
            try { if (typeof window._renderComissoesPatch === 'function') window._renderComissoesPatch(); } catch (_) {}
          } catch (e) {
            hotfixToast('Erro ao salvar OF: ' + String(e && e.message || e), 'var(--red)');
          } finally {
      ...
      body.innerHTML = '<div style="padding:16px;color:#f87171">Erro ao abrir modal: ' + esc(String(e && e.message || e)) + '</div>';
    }
  }

  function bindTrocarDelegationPatch() {
    if (window.__com2TrocarBound) return;
    window.__com2TrocarBound = true;
    document.addEventListener('click', function(e) {
      try {
        var btn = e && e.target && (e.target.closest ? e.target.closest('button[data-com-trocar], button._btn_editar_of, ._btn_editar_of') : null);
        if (!btn) return;
        var id = String(btn.getAttribute('data-of-id') || btn.dataset.ofId || '').trim();
        if (!id) return;
        e.preventDefault();
        e.stopPropagation();
        if (typeof e.stopImmediatePropagation === 'function') e.stopImmediatePropagation();
        openComissoesTrocarModal(id);
      } catch (_) {}
    }, true);
  }

  function refreshComissoesFallbacks() {
    try {
      var data = window._comissoesSqlData || {};
      var ofs = Array.isArray(data.ofs) ? data.ofs : (Array.isArray(window._comissaoOFs) ? window._comissaoOFs : []);
      var vendedores = Array.isArray(data.vendedores) ? data.vendedores : [];
      var vendPctMap = Object.create(null);
      ...
      Array.prototype.slice.call(document.querySelectorAll('#_com_topo div')).forEach(function(el) {
        ...
        if (txt === 'total vendido') {
          var valEl = el.parentElement && el.parentElement.querySelector('div:nth-child(2)');
          if (valEl) valEl.textContent = money(totalVend);
        }
        if (txt === 'total comissoes') {
          var valEl2 = el.parentElement && el.parentElement.querySelector('div:nth-child(2)');
          if (valEl2) valEl2.textContent = money(totalCom);
        }
      });
      ...
      Array.prototype.slice.call(document.querySelectorAll('tr[data-of-row="1"]')).forEach(function(tr) {
        ...
        var numero = String(tr.getAttribute('data-of-numero') || '').trim();
        var of = map[id] || map[numero];
        if (!of) return;
        ...
        tds[9].innerHTML = '<span style="background:' + (isConcluidaStatus(of.status) ? '#166534' : '#334155') + ';color:' + (isConcluidaStatus(of.status) ? '#4ade80' : '#e2e8f0') + ';padding:2px 8px;border-radius:4px;font-size:11px">' + esc(of.status) + '</span>';
      });
    } catch (_) {}
  }

  function wrapRenderComissoesPatch() {
    if (typeof window._renderComissoesPatch !== 'function' || window._renderComissoesPatch._com2Wrapped) return;
    var orig = window._renderComissoesPatch;
    var wrapped = async function() {
      var r = await orig.apply(this, arguments);
      try { refreshComissoesFallbacks(); } catch (_) {}
      try { bindTrocarDelegationPatch(); } catch (_) {}
      return r;
    };
    wrapped._com2Wrapped = true;
    window._renderComissoesPatch = wrapped;
  }

  function menuItemContainer(el, sidebar) {
    var cur = el;
    for (var i = 0; i < 8 && cur && cur !== sidebar; i += 1) {
      if (/^(li|div)$/i.test(cur.tagName || '') && cur !== el) return cur;
  ...
  function menuBelongsFinanceiro(item, sidebar) {
    try {
      var cur = item;
      for (var i = 0; i < 10 && cur && cur !== sidebar; i += 1) {
        if (normText(cur.textContent).indexOf('financeiro') >= 0) return true;
        cur = cur.parentElement;
      }
      var prev = item;
      for (var j = 0; j < 6 && prev; j += 1) {
        prev = prev.previousElementSibling;
        if (!prev) break;
        var txt = normText(prev.textContent);
        if (txt.indexOf('financeiro') >= 0) return true;
      }
    } catch (_) {}
    return false;
  }

  function removeCaixasPerdidasDuplicada() {
    try {
      var sidebar = document.querySelector('.sidebar,[class*="sidebar"]');
      if (!sidebar) return;
      var matches = [];
      Array.prototype.slice.call(sidebar.querySelectorAll('*')).forEach(function(el) {
        var txt = normText(el && el.textContent);
        if (txt.indexOf('caixas perdidas') < 0) return;
        var item = menuItemContainer(el, sidebar);
        if (!item) return;
        matches.push({ el: el, item: item, financeiro: menuBelongsFinanceiro(item, sidebar) });
      });
      if (!matches.length) return;
      var keep = matches.find(function(m) { return m.financeiro; }) || matches[0];
      matches.forEach(function(m) {
        if (m.item === keep.item) return;
  ...
  function bindSidebarCleanup() {
    function run() {
      try { removeRelatorioMensal(); } catch (_) {}
      try { removeCaixasPerdidasDuplicada(); } catch (_) {}
    }
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', run, { once: true });
    else run();
    function ensureObs() {
      var sidebar = document.querySelector('.sidebar,[class*="sidebar"]');
      if (!sidebar || sidebar._menuCleanupObs) return;
      var obs = new MutationObserver(function() { run(); });
      obs.observe(sidebar, { childList: true, subtree: true });
      sidebar._menuCleanupObs = obs;
    }
    ensureObs();
    setTimeout(ensureObs, 1200);
    setTimeout(run, 1800);
  }
  ...
  async function recalcToneladasPatch() {
    ...
    try {
      ...
      var gramMap = Object.create(null);
      (Array.isArray(gramList) ? gramList : []).forEach(function(g) {
      ...
      var porCli = Object.create(null);
      detalhes.forEach(function(d) {
        var gramKey = String(d.gram || 0) + ' g/m²';
        if (!porGram[gramKey]) porGram[gramKey] = { gram: d.gram || 0, ofs: 0, area_m2: 0, toneladas: 0 };
        porGram[gramKey].ofs += 1;
        porGram[gramKey].area_m2 += toNum(d.area_m2);
        porGram[gramKey].toneladas += toNum(d.toneladas);
        var cliKey = String(d && (d.cliente || d.cli_nome || d.cliente_nome || '—') || '—');
        if (!porCli[cliKey]) porCli[cliKey] = { cliente: cliKey, ofs: 0, area_m2: 0, toneladas: 0 };
        porCli[cliKey].ofs += 1;
        porCli[cliKey].area_m2 += toNum(d.area_m2);
        porCli[cliKey].toneladas += toNum(d.toneladas);
      });
      var totalTon = detalhes.reduce(function(s, d) { return s + toNum(d.toneladas); }, 0);
      var totalM2 = detalhes.reduce(function(s, d) { return s + toNum(d.area_m2); }, 0);
      ...
      if (cardTon) cardTon.textContent = totalTon.toFixed(4) + ' t';
      if (cardM2) cardM2.textContent = totalM2.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' m²';
      if (cardOfs) cardOfs.textContent = String(detalhes.length);
      var porGramRows = Object.values(porGram).sort(function(a, b) { return b.toneladas - a.toneladas; });
      var porCliRows = Object.values(porCli).sort(function(a, b) { return b.toneladas - a.toneladas; });
      var html = ''
        + '<h3 style="color:var(--text,#fff);margin:20px 0 10px">Por Gramatura</h3>'
        + '<div style="overflow:auto;border-radius:12px;border:1px solid var(--border,#333);margin-bottom:20px"><table style="width:100%;border-collapse:collapse"><thead><tr style="background:var(--bg2,#1a1a2e)"><th style="padding:10px;text-align:right;color:#aaa;font-size:11px">GRAMATURA</th><th style="padding:10px;text-align:right;color:#aaa;font-size:11px">OFs</th><th style="padding:10px;text-align:right;color:#aaa;font-size:11px">M²</th><th style="padding:10px;text-align:right;color:#aaa;font-size:11px">TONELADAS</th></tr></thead><tbody>'
        + (porGramRows.length ? porGramRows.map(function(g, i) {
          return '<tr style="border-top:1px solid var(--border,#333);background:' + (i % 2 ? 'rgba(255,255,255,0.02)' : 'transparent') + '"><td style="padding:8px 10px;text-align:right;color:#6366f1;font-weight:700">' + esc(String(g.gram || 0)) + ' g/m²</td><td style="padding:8px 10px;text-align:right;color:#aaa">' + esc(String(g.ofs || 0)) + '</td><td style="padding:8px 10px;text-align:right;color:#aaa">' + esc(g.area_m2.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })) + ' m²</td><td style="padding:8px 10px;text-align:right;color:#10b981;font-weight:700">' + esc(g.toneladas.toFixed(4)) + ' t</td></tr>';
        }).join('') : '<tr><td colspan="4" style="padding:16px;text-align:center;color:#94a3b8">Sem dados no período.</td></tr>')
        + '</tbody></table></div>'
        + '<h3 style="color:var(--text,#fff);margin:20px 0 10px">Top Clientes por Toneladas</h3>'
        + '<div style="overflow:auto;border-radius:12px;border:1px solid var(--border,#333);margin-bottom:20px"><table style="width:100%;border-collapse:collapse"><thead><tr style="background:var(--bg2,#1a1a2e)"><th style="padding:10px;text-align:left;color:#aaa;font-size:11px">CLIENTE</th><th style="padding:10px;text-align:right;color:#aaa;font-size:11px">OFs</th><th style="padding:10px;text-align:right;color:#aaa;font-size:11px">M²</th><th style="padding:10px;text-align:right;color:#aaa;font-size:11px">TONELADAS</th></tr></thead><tbody>'
        + (porCliRows.length ? porCliRows.map(function(c, i) {
          return '<tr style="border-top:1px solid var(--border,#333);background:' + (i % 2 ? 'rgba(255,255,255,0.02)' : 'transparent') + '"><td style="padding:8px 10px;color:var(--text,#fff)">' + esc(c.cliente) + '</td><td style="padding:8px 10px;text-align:right;color:#aaa">' + esc(String(c.ofs || 0)) + '</td><td style="padding:8px 10px;text-align:right;color:#aaa">' + esc(c.area_m2.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })) + ' m²</td><td style="padding:8px 10px;text-align:right;color:#10b981;font-weight:700">' + esc(c.toneladas.toFixed(4)) + ' t</td></tr>';
        }).join('') : '<tr><td colspan="4" style="padding:16px;text-align:center;color:#94a3b8">Sem dados no período.</td></tr>')
        + '</tbody></table></div>'
      ...
      if (corpo) corpo.innerHTML = '<div style="color:#f87171;padding:20px">Erro: ' + esc(String(e && e.message || e)) + '</div>';
    }
  }

  function wrapToneladasPatch() {
    window._recarregarToneladas = recalcToneladasPatch;
  }

  async function cloneOfViaPatch() {
    var ctx = window._OF_CLONE_CTX;
    if (!ctx || !ctx.active) return false;
    var origemId = String(ctx.origemId || '').trim();
    if (!origemId) throw new Error('OF de origem inválida para clonagem');
    var out = await fetchJsonPatch('/api/ofs/' + encodeURIComponent(origemId), { headers: tokenHeaders() });
    ...
    try {
      ...
      maq = checked.length ? checked : null;
    } catch (_) {}
    if (!maq) {
      if (Array.isArray(of.maq)) maq = of.maq.slice();
      else if (typeof of.maq === 'string') {
        try { maq = JSON.parse(of.maq || '[]'); } catch (_) { maq = String(of.maq || '').trim() ? [String(of.maq || '').trim()] : []; }
      } else maq = [];
    }
    var body = {
    ...
    var save = await fetchJsonPatch('/api/ofs', {
      method: 'POST',
      headers: tokenHeaders({ 'Content-Type': 'application/json' }),
      body: JSON.stringify(body)
    });
    if (!save.response.ok || !save.json || save.json.ok === false) {
      throw new Error('HTTP ' + save.response.status + ' - ' + String(save.json && (save.json.error || save.json.message) || 'Falha ao clonar OF'));
    }
    var nova = (save.json && (save.json.data || save.json)) || {};
    try { if (typeof window.fechar === 'function') window.fechar('modal-of'); } catch (_) {}
    try { window._OF_CLONE_CTX = null; } catch (_) {}
    hotfixToast('OF clonada com sucesso — Nº ' + String(nova.numero || nova.of || '—'), 'var(--green)');
    try {
      if (typeof window.carregarOFs === 'function') await window.carregarOFs({ forcar: true });
    } catch (_) {}
    try { if (typeof window.renderPCP === 'function') window.renderPCP(); } catch (_) {}
    return true;
  }

  function wrapSalvarClonePatch() {
    if (typeof window.salvarOF !== 'function' || window.salvarOF._cloneWrappedHotfix) return;
    var orig = window.salvarOF;
    window.salvarOF = async function() {
      try {
        if (window._OF_CLONE_CTX && window._OF_CLONE_CTX.active) {
          return await cloneOfViaPatch();
        }
      } catch (e) {
        hotfixToast(String(e && e.message || e), 'var(--red)');
        return false;
      }
      return orig.apply(this, arguments);
    };
    window.salvarOF._cloneWrappedHotfix = true;
  }
  ...
  function pcpStatusMeta(of) {
    ...
    if (concluida) return { label: 'Concluída', color: '#22C55E', opacity: 0.7 };
    return { label: 'Em Aberto', color: '#3B82F6', opacity: 1 };
  }
  ...
  function getPcpRowsAll() {
    var all = Array.isArray(window.OFs) ? window.OFs.slice() : (Array.isArray(window.OFS) ? window.OFS.slice() : []);
    return all;
  }

  function getPcpScopeRows(all) {
    var busca = String((document.getElementById('pcp-busca') || {}).value || '').trim().toLowerCase();
    var vend = String((document.getElementById('pcp-vend') || {}).value || '').trim();
    var emp = String((document.getElementById('pcp-emp') || {}).value || '').trim();
    var diaAtual = String(window.PCP_DIA_ATUAL || '').trim();
    var isTodas = diaAtual === '__todas__';
    var isSemana = diaAtual === '__semana__';
    var list = (all || []).filter(function(o) {
      if (!o) return false;
      if (isSemana) {
        return true;
      }
      if (isTodas || !diaAtual) return true;
      var dia = String(o.dia || o.data_producao || '').slice(0, 10);
      var ent = String(o.ent || o.data_entrega || '').slice(0, 10);
      var concl = String(o.data_conclusao || '').slice(0, 10);
      return dia === diaAtual || ent === diaAtual || concl === diaAtual || String(o.status || '').toLowerCase().indexOf('produ') >= 0 || (ent && ent < diaAtual);
    });
    if (busca) {
      list = list.filter(function(o) {
        var cli = '';
        try { cli = String((typeof getCli === 'function' ? ((getCli(o.cliId || o.cli_id || o.cliente_id) || {}).nome) : '') || o.cliNome || o.cliente_nome || o.cliente || '').toLowerCase(); } catch (_) {}
        var blob = [o.of, o.numero, cli, o.prodDesc, o.descricao].map(function(v) { return String(v || '').toLowerCase(); }).join(' ');
        return blob.indexOf(busca) >= 0;
      });
    }
    if (vend) list = list.filter(function(o) { return String(o.vendId || o.vendedor_id || '').trim() === vend; });
    if (emp) list = list.filter(function(o) { return String(o.empId || o.emp_id || o.empresa_id || '').trim() === emp; });
    ...
    list.sort(function(a, b) {
      var ma = pcpStatusMeta(a);
      var mb = pcpStatusMeta(b);
      if (ma.label !== mb.label) {
        var rank = { 'Urgente': 0, 'Atrasada': 1, 'Em Aberto': 2, 'Concluída': 3, 'Excluída': 4 };
        return (rank[ma.label] || 9) - (rank[mb.label] || 9);
      }
      var da = String(a.ent || a.data_entrega || a.dia || a.created_at || '');
      var db = String(b.ent || b.data_entrega || b.dia || b.created_at || '');
      if (da !== db) return da.localeCompare(db);
      return String(a.of || a.numero || '').localeCompare(String(b.of || b.numero || ''));
    });
    return list;
  }

  function buildPcpStatusTag(meta) {
    return '<span style="display:inline-block;padding:4px 10px;border-radius:999px;background:' + meta.color + '20;color:' + meta.color + ';border:1px solid ' + meta.color + ';font-size:11px;font-weight:700">' + esc(meta.label) + '</span>';
  }
  ...
  function patchPcpRowsAndStatus() {
    ...
    if (resumoEl) {
      ...
      resumoEl.textContent = String(list.length) + ' OF(s) · ' + String(concl) + ' concluída(s) · ' + String(excl) + ' excluída(s)';
    }
    tbody.innerHTML = renderList.length ? renderList.map(function(o) {
      var meta = pcpStatusMeta(o);
      var cli = (typeof getCli === 'function' ? (getCli(o.cliId || o.cli_id || o.cliente_id) || {}) : {});
      var vend = (typeof getV === 'function' ? (getV(o.vendId || o.vendedor_id || o.vend_id) || {}) : {});
      var cliTxt = String(cli.nome || o.cliNome || o.cliente || o.cliente_nome || '—');
      ...
      return '<tr data-of-id="' + esc(String(o.id || '')) + '" data-status-visual="' + esc(meta.label) + '" style="' + rowBg + '" onclick="selecionarOF && selecionarOF(\'' + esc(String(o.of || o.numero || '')) + '\')">'
        ...
        + '<td class="tc">' + (imgPrintUrl ? '<button class="btn-icon" onclick="event.stopPropagation();imprimirImagemOF && imprimirImagemOF(\'' + esc(String(o.id || o.of || '')) + '\')" title="Imprimir imagem">🖼</button>' : '<span class="img-ph" onclick="event.stopPropagation()">📷</span>') + '</td>'
        + '<td>' + esc(cliTxt) + (typeof empBadge === 'function' ? ' ' + empBadge(o.empId || o.emp_id || o.empresa_id) : '') + '</td>'
        + '<td style="color:var(--text2)">' + esc(vendTxt) + '</td>'
        + '<td class="tc">' + esc(String(qtd.toLocaleString('pt-BR'))) + '</td>'
        + '<td class="tc" style="font-family:var(--mono);font-weight:900">' + esc(unitVal > 0 ? money(unitVal) : '—') + '</td>'
        + '<td class="tc" style="font-family:var(--mono);font-weight:900;color:var(--green)">' + esc(totalVal > 0 ? money(totalVal) : '—') + '</td>'
        + mCells
        ...
        + '</div></td>'
        + '</tr>';
    }).join('') : '<tr><td colspan="19" style="text-align:center;padding:32px;color:var(--text3)">Nenhuma OF encontrada nos filtros selecionados.</td></tr>';
    try { if (typeof window._pcpSyncPaginacaoUI === 'function') window._pcpSyncPaginacaoUI(); } catch (_) {}
  }

  function wrapRenderPcpPatch() {
    if (typeof window.renderPCP !== 'function' || window.renderPCP._statusWrappedHotfix) return;
    var orig = window.renderPCP;
    ...
    wrapped._statusWrappedHotfix = true;
    window.renderPCP = wrapped;
  }

  function injectMobilePatch() {
    try {
      if (!document.querySelector('meta[name="viewport"]')) {
        document.head.insertAdjacentHTML('afterbegin', '<meta name="viewport" content="width=device-width, initial-scale=1">');
      }
    } catch (_) {}
    if (!document.getElementById('patch-mobile-responsive-style')) {
      var style = document.createElement('style');
      style.id = 'patch-mobile-responsive-style';
      style.textContent = ''
        + '#btn-menu-mobile{position:fixed;top:10px;left:10px;z-index:1002;display:none;align-items:center;justify-content:center;width:44px;height:44px;border:none;border-radius:12px;background:#0f172a;color:#fff;box-shadow:0 10px 30px rgba(0,0,0,.3)}'
        + '#sidebar-overlay{position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:999;display:none}'
        + '@media (max-width:767px){'
        + '.sidebar,#sidebar,[class*="sidebar"]{position:fixed!important;top:0;left:0;bottom:0;transform:translateX(-100%);z-index:1000;transition:transform .3s ease;max-width:84vw!important;width:84vw!important}'
        + '.sidebar.sidebar-open,#sidebar.sidebar-open,[class*="sidebar"].sidebar-open{transform:translateX(0)!important}'
        + '#content,.main-content,#main-content,[class*="main-content"]{margin-left:0!important;width:100%!important;padding-top:60px!important}'
        + '#btn-menu-mobile{display:flex!important}'
        + '#sidebar-overlay.sidebar-open{display:block!important}'
        + 'table{min-width:600px}'
        + 'table.pcp-table,table{display:table}'
        + 'table.pcp-table{min-width:1200px}'
        + '.table-wrap,.table-responsive,#pcp-table-wrap,#ton-corpo,#_com_detalhe{overflow-x:auto!important}'
        + '.modal,.m-box,[class*="modal"] .modal-content{width:95vw!important;max-width:95vw!important;max-height:90vh!important;overflow-y:auto!important}'
        + 'button,.btn,.pcp-btn,.btn-icon{min-height:44px}'
        + '}'
        + '@media (max-width:479px){.cards,.dashboard-cards,[class*="cards"]{flex-direction:column!important;grid-template-columns:1fr!important}}';
      document.head.appendChild(style);
    }
    if (!document.getElementById('btn-menu-mobile')) {
      ...
      btn.onclick = function() {
        ...
        if (overlay) overlay.classList.toggle('sidebar-open', open);
      };
  ...
  bindTrocarDelegationPatch();
  bindSidebarCleanup();
  wrapToneladasPatch();
  wrapSalvarClonePatch();
  wrapConclusaoImagemPatch();
  wrapRenderPcpPatch();
  injectMobilePatch();
  setTimeout(function() {
    try { wrapRenderComissoesPatch(); } catch (_) {}
    try { refreshComissoesFallbacks(); } catch (_) {}
    try { patchPcpRowsAndStatus(); } catch (_) {}
  }, 200);
  setTimeout(function() {
  ...
  try { window.removeRelatorioMensal = removeRelatorioMensal; } catch (_) {}
  try { window.removeCaixasPerdidasDuplicada = removeCaixasPerdidasDuplicada; } catch (_) {}
})();
setTimeout(function() {
...
function _injetarGramaturaNoModal() {
  if (window.__patchConclusaoGramaturaFetch !== true && typeof window.fetch === 'function') {
    window.__patchConclusaoGramaturaFetch = true;
    var origFetchConclusaoGram = window.fetch;
    window.fetch = function(resource, init) {
      try {
        var url = typeof resource === 'string' ? resource : (resource && resource.url) || '';
        if (/\/api\/ofs\/[^/]+\/(concluir|baixa)(\?|$)/i.test(String(url || ''))) {
          var opts = init ? Object.assign({}, init) : {};
          var gramId = String(window._ofGramaturaIdPendente || '').trim();
      ...
      return origFetchConclusaoGram.call(this, resource, init);
    };
  }
  if (window.__gramaturaModalObs) return;
  var obs = new MutationObserver(function() {
    var modal = document.getElementById('modal-concluir-of');
    if (!modal || modal.querySelector('#_gram_select_modal')) return;
    if (modal.style.display === 'none' || modal.hidden) return;

    var inputs = modal.querySelectorAll('input[type="number"], input:not([type="checkbox"])');
    var pontoInsercao = inputs[0] ? (inputs[0].closest ? inputs[0].closest('div, p, label, .m-field, .field') : null) : null;
    if (!pontoInsercao) pontoInsercao = modal.firstElementChild;

    var divGram = document.createElement('div');
    divGram.style.cssText = 'margin:12px 0';
    divGram.innerHTML =
      '<label style="display:block;font-size:11px;font-weight:700;text-transform:uppercase;'
      + 'letter-spacing:.5px;color:var(--text2,#94a3b8);margin-bottom:6px">Gramatura (papel utilizado)</label>'
      + '<select id="_gram_select_modal" style="width:100%;background:var(--bg2,#1e293b);'
      + 'border:1px solid var(--border,#334155);border-radius:8px;padding:12px;'
      + 'color:var(--text1,#f1f5f9);font-size:14px">'
      + '<option value="">— Sem gramatura —</option>'
      + '</select>';

    if (pontoInsercao && pontoInsercao.parentNode) {
      pontoInsercao.parentNode.insertBefore(divGram, pontoInsercao.nextSibling);
    } else {
      modal.appendChild(divGram);
    }
    ...
    fetch('/api/gramaturas')
      ...
      .then(function(lista) {
        ...
        console.log('[GRAM MODAL] ' + lista.length + ' gramaturas carregadas');
      }).catch(function(e) { console.warn('[GRAM MODAL] erro:', e.message); });

    var btnConf = modal.querySelector('#btn-conf-conc, button[onclick*="confirmar"], .btn-accent');
    if (btnConf && !btnConf._gramPatch) {
      btnConf._gramPatch = true;
      btnConf.addEventListener('click', function() {
        var sel = document.getElementById('_gram_select_modal');
        if (sel && sel.value) {
          window._ofGramaturaIdPendente = sel.value;
          console.log('[GRAM MODAL] gramatura_id salvo:', sel.value);
        } else {
          window._ofGramaturaIdPendente = '';
        }
      }, true);
    }
  });
  window.__gramaturaModalObs = obs;
...
if (!document.getElementById('_patch_light_css')) {
  ...
  s.textContent = 'body._patch_modo_claro{background:#f0f0f0!important}' +
    'body._patch_modo_claro *{color:#111!important}' +
    'body._patch_modo_claro input,body._patch_modo_claro select,body._patch_modo_claro textarea{background:#fff!important;border-color:#ccc!important;color:#111!important}' +
    'body._patch_modo_claro td,body._patch_modo_claro th{background:#fff!important;border-color:#ccc!important;color:#111!important}' +
    'body._patch_modo_claro .sidebar,body._patch_modo_claro [class*="sidebar"],body._patch_modo_claro nav,body._patch_modo_claro [class*="nav"]{background:#dde!important}' +
    'body._patch_modo_claro button{color:#111!important}' +
    'body._patch_modo_claro [style*="color:#"]{color:#111!important}' +
    'body._patch_modo_claro [style*="color:var"]{color:#111!important}' +
    'body._patch_modo_claro [style*="background:#0"],body._patch_modo_claro [style*="background:#1"],body._patch_modo_claro [style*="background:#2"],body._patch_modo_claro [style*="background:rgba(0"],body._patch_modo_claro [style*="background:rgba(1"]{background:#fff!important}';
  document.head.appendChild(s);
}
...
(function() {
  ...
  window.__patchCloneOF = async function(ofId, buscaModal) {
    try {
      ...
      delete payload.num_of;
      delete payload.numero_of;
      delete payload.id;
      delete payload.created_at;
      delete payload.updated_at;
      delete payload.data_conclusao;
      delete payload.data_producao;
...
(function initComissoesEditarQtdProduzida() {
  ...
  try { window._parseListaTextoOuJsonHotfix = parseListaTextoOuJson; } catch (_) {}
    ...
    function _cpRender(data) {
      ...
      if (inpB) {
        if (!window.__cpv2BuscaDebounced) {
          window.__cpv2BuscaDebounced = window._debounce(function(valor, start, end) {
            _cpState().busca = String(valor || '');
            _cpRender(data);
            setTimeout(function() {
              var novoInp = document.getElementById('cpv2-busca');
              if (!novoInp) return;
              try { novoInp.focus(); } catch (_) {}
              try { novoInp.setSelectionRange(start, end); } catch (_) {}
            }, 0);
          }, 250);
        }
        inpB.oninput = function() {
          var valor = String(inpB.value || '');
          var start = inpB.selectionStart;
          var end = inpB.selectionEnd;
          window.__cpv2BuscaDebounced(valor, start, end);
        };
      }
...
(function patchCalcularComissoesSqlBackend() {
  ...
  async function _abrirModalImpressao() {
    ...
    var mesesImp = ['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'];
    var mesNumAtual = parseInt(String(mesAtual || '').replace(/\D/g, ''), 10) || (new Date().getMonth() + 1);
    overlay.innerHTML = ''
      ...
      + '<div style="display:flex;gap:8px">'
      + '<select id="_imp_mes" style="flex:1;color:var(--text1,#fff);font-size:14px;padding:10px;background:var(--bg3,#0d0d1a);border-radius:6px;border:1px solid var(--border,#333)">' + mesesImp.map(function(nome, idx) {
          var mesNum = idx + 1;
          return '<option value="' + mesNum + '"' + (mesNum === mesNumAtual ? ' selected' : '') + '>' + nome + '</option>';
        }).join('') + '</select>'
      + '<select id="_imp_ano" style="width:120px;color:var(--text1,#fff);font-size:14px;padding:10px;background:var(--bg3,#0d0d1a);border-radius:6px;border:1px solid var(--border,#333)">' + [2024, 2025, 2026].map(function(ano) {
          return '<option value="' + ano + '"' + (String(anoAtual) === String(ano) ? ' selected' : '') + '>' + ano + '</option>';
        }).join('') + '</select>'
      + '</div>'
      + '</div>'
  ...
  window._gerarImpressaoComissoes = function() {
    (async function() {
      var data = window._comissoesSqlData;
      var mesSel = document.getElementById('_imp_mes');
      var anoSel = document.getElementById('_imp_ano');
      var mesImp = parseInt((mesSel && mesSel.value) || 0, 10) || null;
      var anoImp = String((anoSel && anoSel.value) || '').trim() || null;

      if (mesImp && anoImp) {
        try {
          var tokenFetch = '';
          try { tokenFetch = String(localStorage.getItem('token') || localStorage.getItem('access_token') || sessionStorage.getItem('token') || '').trim(); } catch (_) {}
          var respImp = await fetch('/api/comissoes/relatorio?mes=' + encodeURIComponent(String(mesImp)) + '&ano=' + encodeURIComponent(String(anoImp)), {
            headers: tokenFetch ? { Authorization: 'Bearer ' + tokenFetch } : {}
          });
          var jsonImp = await respImp.json().catch(function() { return null; });
          if (respImp.ok && jsonImp && jsonImp.ok) data = jsonImp;
        } catch (_) {}
      }
      ...
      var parts = String(data.mes || (anoImp + '-' + String(mesImp || '').padStart(2, '0'))).split('-');
      var ano = parts[0] || '';
      var mes = parts[1] || '';
...
(function patchEditarOfPcpAbrirNaOfRapida() {
  ...
  async function fetchOf(id) {
    ...
    if (!sid) return null;
    var token = '';
    try { token = String(localStorage.getItem('token') || localStorage.getItem('access_token') || '').trim(); } catch (_) {}
    var resp = await fetch('/api/ofs/' + encodeURIComponent(sid), {
      headers: token ? { Authorization: 'Bearer ' + token } : {}
    });
    var j = await resp.json().catch(function() { return null; });
    var of = (j && j.data) ? j.data : (j && j.ok ? j : j);
    console.log('[EDITAR OF] dados:', JSON.stringify(of).slice(0, 400));
    return of;
  }
  ...
  function parseMaqValue(of) {
    try {
      var raw = of && (of.maq != null ? of.maq : (of.fluxo_maquinas != null ? of.fluxo_maquinas : null));
      if (Array.isArray(raw) && raw[0]) return String(raw[0] && (raw[0].nome || raw[0].maquina || raw[0]) || '').trim();
      if (typeof raw === 'string') {
        var s = raw.trim();
        if (s.charAt(0) === '[') {
          var parsed = JSON.parse(s || '[]');
          if (Array.isArray(parsed) && parsed[0]) return String(parsed[0] && (parsed[0].nome || parsed[0].maquina || parsed[0]) || '').trim();
        }
        if (s) return s;
      }
    } catch (_) {}
    return String(of && (of.maquina_agendada || of.maquina || of.maquina_atual) || '').trim();
  }

  function setAnyVal(selectors, val) {
    if (val == null) return;
    var list = Array.isArray(selectors) ? selectors : [selectors];
    list.forEach(function(sel) {
      if (!sel) return;
      var el = null;
      try { el = sel.charAt && sel.charAt(0) === '#' ? document.getElementById(sel.slice(1)) : document.querySelector(sel); } catch (_) { el = null; }
      if (!el) return;
      try {
        el.value = String(val);
  ...
  function patchSalvarOfRapida() {
    ...
    window.salvarOfRapida = async function() {
      ...
      if (typeof window.salvarEdicaoOf === 'function') return window.salvarEdicaoOf();
  ...
  function patchAbrirModalOF() {
    ...
    window.abrirModalOF = async function(ofId) {
      ...
      var urgente = !!(of.urgente === true || of.urg === true || of.urgente === 1 || of.urg === 1);

      setAnyVal(['#of-r-cliente', 'input[name="cliente"]'], cliNome);
      ...
      setChecked('of-r-urgente', urgente);

      if (gramId) {
        window._ofRapidaGramaturaId = gramId;
        setTimeout(function() {
          var sel = document.getElementById('_of_rapida_gramatura_id') || document.getElementById('of-r-gramatura');
          if (sel) sel.value = gramId;
        }, 400);
      }

      if (cliNome) {
        var elCli = document.getElementById('of-r-cliente');
        if (elCli) {
          elCli.value = cliNome;
          elCli.dispatchEvent(new Event('input', { bubbles: true }));
          elCli.dispatchEvent(new Event('change', { bubbles: true }));
        }
      }
...
(function patchOfRapidaCoresMultiItemAndSalvarEdicao() {
  ...
  window.salvarEdicaoOf = async function() {
    ...
    var ofOrig = window._ofRapidaOriginalOF || {};
    if (!id) {
    ...
    try {
      ...
      if (!resp.ok || (result && result.ok === false)) {
        toast('❌ Erro ao salvar: ' + String(result && (result.error || result.message) || resp.status));
        return;
      }
      toast('OF atualizada com sucesso');
      window._ofRapidaEditandoId = null;
      window._ofRapidaEditandoNumero = null;
      window._ofRapidaOriginalOF = null;
...
(function patchOfmaqAndHubIntelligence() {
  ...
  function getVisibleGroups() {
    var groups = window._ofmaqVisibleGroups || window._ofmaqLastGroupOfs;
    return (groups && typeof groups === 'object') ? groups : {};
  }

  function getOfmaqRawList() {
    var raw = Array.isArray(window._ofmaqAllOfs) ? window._ofmaqAllOfs : null;
    if (raw && raw.length) return raw.slice();
    raw = Array.isArray(window.OFS) ? window.OFS : (Array.isArray(window._ofmaqBaseList) ? window._ofmaqBaseList : []);
    return Array.isArray(raw) ? raw.slice() : [];
  }

  function getMachineFromOf(of) {
    var maq = '';
    try {
      if (of && of.__maquinaRender) maq = String(of.__maquinaRender || '').trim();
      if (!maq) maq = String(of && (of.maquina_agendada || of.maquina_atual || of.maquina || of.maquina_nome) || '').trim();
      if (!maq && Array.isArray(of && of.maq) && of.maq[0]) maq = String(of.maq[0] || '').trim();
      if (!maq && typeof (of && of.maq) === 'string') {
        try {
          var maqParsed = JSON.parse(String(of.maq || '[]'));
          if (Array.isArray(maqParsed) && maqParsed[0]) maq = String(maqParsed[0] || '').trim();
        } catch (_) {
          maq = String(of && of.maq || '').trim();
        }
      }
    } catch (_) {}
    return maq || 'Sem Máquina';
  }

  function capturarEstadoOfmaqVisivel() {
    try {
      var groups = Object.create(null);
      var all = [];
      var originalOrder = Object.create(null);
      var roots = Array.prototype.slice.call(document.querySelectorAll('.maq-body[data-maquina-id], .maq-ofs[data-maquina-id], [data-maquina-lista="1"][data-maquina-id]'));
      roots.forEach(function(listEl) {
        var maq = String(listEl && (listEl.getAttribute('data-maquina-id') || listEl.getAttribute('data-maquina') || '') || '').trim() || 'Sem Máquina';
        ...
        cards.forEach(function(card) {
          ...
          if (!of) return;
          var clone = Object.assign({}, of, { __maquinaRender: maq });
          if (!groups[maq]) groups[maq] = [];
          groups[maq].push(clone);
          all.push(clone);
        });
        originalOrder[maq] = (groups[maq] || []).map(function(of) { return String(of && of.id || '').trim(); }).filter(Boolean);
      });

      if (!Object.keys(groups).length) {
        getOfmaqRawList().forEach(function(of) {
          var maq = getMachineFromOf(of);
          var clone = Object.assign({}, of, { __maquinaRender: maq });
          if (!groups[maq]) groups[maq] = [];
          groups[maq].push(clone);
          all.push(clone);
        });
        Object.keys(groups).forEach(function(maq) {
          originalOrder[maq] = (groups[maq] || []).map(function(of) { return String(of && of.id || '').trim(); }).filter(Boolean);
        });
      }

      window._ofmaqVisibleGroups = groups;
      window._ofmaqLastGroupOfs = groups;
      window._ofmaqAllOfs = all.slice();
      if (!window._ofmaqAllById || typeof window._ofmaqAllById !== 'object') window._ofmaqAllById = Object.create(null);
      all.forEach(function(of) {
        var sid = String(of && of.id || '').trim();
        if (sid) window._ofmaqAllById[sid] = of;
      });
      if (!window._ofmaqOrderMode || window._ofmaqOrderMode === 'original' || !window._ofmaqOriginalOrderByMachine) {
        window._ofmaqOriginalOrderByMachine = originalOrder;
      }
    } catch (_) {}
  }
  ...
  function ensureOfmaqToolbarButtons() {
    ...
    if (!document.getElementById('btn-resetar-ordem-ofmaq')) {
      var btnReset = document.createElement('button');
      btnReset.id = 'btn-resetar-ordem-ofmaq';
      btnReset.style.cssText = 'padding:7px 14px;border-radius:6px;background:transparent;color:var(--text1);border:1px solid var(--border);cursor:pointer;font-size:13px';
      btnReset.textContent = '↺ Resetar';
      btnReset.onclick = function() { try { if (typeof window._resetarOrdemOfmaq === 'function') window._resetarOrdemOfmaq(); } catch (_) {} };
      wrap.appendChild(btnReset);
    }
    updatePrioridadeButton();
  ...
  function updatePrioridadeButton() {
    var btn = document.getElementById('patch-btn-prioridade');
    if (!btn) return;
    var ativo = String(window._ofmaqOrderMode || 'original') === 'medidas';
    btn.style.background = ativo ? 'var(--green)' : 'var(--accent)';
    btn.textContent = ativo ? '🎯 Ordenado por Medidas ✓' : '🎯 Ordenar por Prioridade';
  }
  ...
  function parseColors(of) {
    var raw = of && (of.cores_impressao != null ? of.cores_impressao : (of.impressao_cor != null ? of.impressao_cor : (of.cores != null ? of.cores : (of.cor_tinta != null ? of.cor_tinta : (of.cor != null ? of.cor : of.especificacoes)))));
    var arr = [];
    try {
      if (Array.isArray(raw)) {
  ...
  function getDimensoesOfmaq(of) {
    var dim = parseDimensions(of);
    return {
      comp: parseFloat(String((of && (of.comp != null ? of.comp : (of.comprimento != null ? of.comprimento : (of.caixa_comprimento != null ? of.caixa_comprimento : dim.comprimento)))) || 0).replace(',', '.')) || 0,
      larg: parseFloat(String((of && (of.larg != null ? of.larg : (of.largura != null ? of.largura : (of.caixa_largura != null ? of.caixa_largura : dim.largura)))) || 0).replace(',', '.')) || 0
    };
  }

  function agruparPorMedidasSemelhantes(list) {
    var items = (Array.isArray(list) ? list.slice() : []).sort(sortByEntregaAsc);
    var grupos = [];
    items.forEach(function(of) {
      var dim = getDimensoesOfmaq(of);
      var target = null;
      for (var i = 0; i < grupos.length; i += 1) {
        var ref = grupos[i].ref;
        if (Math.abs((ref.comp || 0) - (dim.comp || 0)) <= 20 && Math.abs((ref.larg || 0) - (dim.larg || 0)) <= 20) {
          target = grupos[i];
          break;
        }
      }
    ...
    return grupos.reduce(function(acc, grupo) {
      return acc.concat(grupo.items.sort(sortByEntregaAsc));
    }, []);
  }

  function getOfmaqColorGroup(of) {
    var cores = parseColors(of).map(function(c) { return String(c || '').trim().toUpperCase(); }).filter(Boolean);
    return cores.length ? cores.sort().join(' + ') : 'SEM COR';
  }

  function buildGroupsFromAllOfs() {
    var groups = Object.create(null);
    getOfmaqRawList().forEach(function(of) {
      var maq = getMachineFromOf(of);
      if (!groups[maq]) groups[maq] = [];
  ...
  function aplicarOrdemOfmaq(groups) {
    var g = (groups && typeof groups === 'object' && Object.keys(groups).length) ? groups : buildGroupsFromAllOfs();
    if (!Object.keys(g).length) return false;
    if (!window._ordemMaquinas || typeof window._ordemMaquinas !== 'object') window._ordemMaquinas = {};
    Object.keys(g).forEach(function(maq) {
      var ids = (Array.isArray(g[maq]) ? g[maq] : []).map(function(of) { return String(of && of.id || '').trim(); }).filter(Boolean);
      if (ids.length) window._ordemMaquinas[maq] = ids.slice();
    });
    window._ofmaqVisibleGroups = g;
    window._ofmaqLastGroupOfs = g;
    window._ofmaqAllOfs = Object.keys(g).reduce(function(acc, maq) {
      return acc.concat((g[maq] || []).map(function(of) { return Object.assign({}, of, { __maquinaRender: maq }); }));
    }, []);
    return true;
  }

  window._resetarOrdemOfmaq = async function() {
    try {
      window._ofmaqOrderMode = 'original';
      window._agrupamentoSetupAtivo = false;
      if (window._ordemMaquinas && typeof window._ordemMaquinas === 'object') {
        Object.keys(window._ordemMaquinas).forEach(function(k) { delete window._ordemMaquinas[k]; });
      }
      var original = window._ofmaqOriginalOrderByMachine || {};
      Object.keys(original).forEach(function(maq) {
        var ids = Array.isArray(original[maq]) ? original[maq].slice() : [];
        if (ids.length) window._ordemMaquinas[maq] = ids;
      });
      updatePrioridadeButton();
      updateAgrupamentoButton();
      try { if (typeof window.renderOFsPorMaquina === 'function') await window.renderOFsPorMaquina(); } catch (_) {}
      try { window.toast('Ordem original restaurada', 'var(--green)'); } catch (_) {}
    } catch (e) {
      try { window.toast('Erro ao resetar ordem: ' + String(e && e.message || e), 'var(--red)'); } catch (_) {}
    }
  };
  ...
  window._ordenarPorPrioridade = async function() {
    if (String(window._ofmaqOrderMode || 'original') === 'medidas') {
      return window._resetarOrdemOfmaq();
    }
    capturarEstadoOfmaqVisivel();
    var groups = buildGroupsFromAllOfs();
    if (!Object.keys(groups).length) {
      try { window.toast('Nenhuma OF carregada. Vá para OFs por Máquina primeiro.', 'var(--red)'); } catch (_) {}
      return;
    }
    var orderedGroups = Object.create(null);
    Object.keys(groups).forEach(function(maq) {
      orderedGroups[maq] = agruparPorMedidasSemelhantes(groups[maq] || []);
    });
    aplicarOrdemOfmaq(orderedGroups);
    window._ofmaqOrderMode = 'medidas';
    window._agrupamentoSetupAtivo = false;
    updatePrioridadeButton();
    updateAgrupamentoButton();
    try { if (typeof window.renderOFsPorMaquina === 'function') await window.renderOFsPorMaquina(); } catch (_) {}
    try { window.toast('✅ OFs ordenadas por medidas similares', 'var(--green)'); } catch (_) {}
  };
    ...
    try {
      ...
      for (var i = 0; i < maquinas.length; i += 1) {
        ...
        keys.sort(function(a, b) {
          if (a === 'SEM COR' && b !== 'SEM COR') return 1;
          if (b === 'SEM COR' && a !== 'SEM COR') return -1;
          return a.localeCompare(b);
        });
        var ordered = [];
        keys.forEach(function(k) {
          var list = (buckets[k] || []).slice().sort(sortByEntregaAsc);
          list.forEach(function(of) { ordered.push(of); });
        });
        groups[maq] = ordered;
      }
      aplicarOrdemOfmaq(groups);
      window._ofmaqOrderMode = 'setup';
  ...
  window.toggleAgrupamentoSetup = async function() {
    if (String(window._ofmaqOrderMode || 'original') === 'setup') {
      return window._resetarOrdemOfmaq();
    }
    return window._agruparPorSetup();
  };
  ...
  function afterRenderOfmaq() {
    ...
    try { _removerBtnNovaOfMaq(); } catch (_) {}
    try { capturarEstadoOfmaqVisivel(); } catch (_) {}
    decorateOfmaqCards();
    ...
    try {
      ...
      var wrapped = async function() {
        ...
        if (!skip) {
          ...
          if (String(mensagem || '').indexOf('Concluída') !== -1 || String(fnName || '').toLowerCase().indexOf('concluir') !== -1) {
            setTimeout(function() {
              if (typeof window.carregarOFSArquivo === 'function') window.carregarOFSArquivo();
              else if (typeof window.renderPCP === 'function') window.renderPCP();
              else if (typeof window.atualizarOFs === 'function') window.atualizarOFs();
              else if (typeof window.carregarOFs === 'function') window.carregarOFs(true);
            }, 1500);
          }
        }
        return result;
...
(function() {
  ...
  async function _abrirModalConclusaoFallback(ofId, ofDados) {
    try {
      ...
      (async function() {
        try {
          var tk = _getToken();
          var rg = await fetch('/api/gramaturas', {
            headers: tk ? { Authorization: 'Bearer ' + tk } : {}
          });
          var jg = await rg.json().catch(function() { return null; });
          var lg = Array.isArray(jg) ? jg : (jg && jg.data || []);

          window._gramaturasConclusaoLista = lg;

          var inpBusca = document.getElementById('conclusao-gramatura-busca');
          var hiddenId = document.getElementById('conclusao-gramatura');
          var lista = document.getElementById('conclusao-gramatura-lista');

          if (!inpBusca || !hiddenId || !lista) return;

          if (of && of.gramatura_id) {
            var gramAtual = lg.find(function(g) { return g.id === of.gramatura_id; });
            if (gramAtual) {
              inpBusca.value = gramAtual.nome || '';
              hiddenId.value = gramAtual.id || '';
            }
          }
          ...
          function mostrarSugestoes(termo) {
            ...
            var filtrados = t.length === 0 ? lg : lg.filter(function(g) {
              return String(g.nome || '').toLowerCase().indexOf(t) >= 0 ||
                     String(g.gramatura || '').indexOf(t) >= 0;
            });
            ...
            lista.innerHTML = filtrados.slice(0, 15).map(function(g) {
              var partes = String(g.nome || '').split(' - ');
              var forn = partes.length >= 3 ? partes[partes.length - 1] : '';
              return '<div data-id="' + escAttr(g.id) + '" data-nome="' + escAttr(g.nome) + '" '
                ...
                + '<div>'
                + '<div style="color:#f1f5f9;font-size:13px">' + String(g.nome || '—').replace(/</g, '&lt;') + '</div>'
                + (forn ? '<div style="color:#64748b;font-size:11px">' + String(forn).replace(/</g, '&lt;') + '</div>' : '')
                + '</div>'
                + '<span style="color:#6366f1;font-weight:700;font-size:13px">' + String(g.gramatura || '') + ' g/m²</span>'
                + '</div>';
            }).join('');

            Array.from(lista.querySelectorAll('[data-id]')).forEach(function(item) {
              item.onmousedown = function(e) {
                e.preventDefault();
                inpBusca.value = item.getAttribute('data-nome') || '';
          ...
          inpBusca.addEventListener('input', function() {
            hiddenId.value = '';
            mostrarSugestoes(inpBusca.value);
          });
          inpBusca.addEventListener('focus', function() {
            mostrarSugestoes(inpBusca.value);
          });
          inpBusca.addEventListener('blur', function() {
            setTimeout(function() { lista.style.display = 'none'; }, 200);
          });
        } catch (e) { console.error('[gram autocomplete]', e); }
      })();
    ...
    try {
      ...
      if (btnSave && !btnSave.dataset.boundSave) {
        ...
        btnSave.addEventListener('click', async function(e) {
          ...
          var cliNome = String((cliInput && cliInput.value) || (of && (of.cli_nome || of.cliente_nome || of.cliente || of.cliNome)) || '').trim();
          ...
          payload.status = st || String(of && of.status || 'Indefinido');
          payload.cliente = cliNome || '—';
          if (cliId) payload.cli_id = cliId;
          payload.vendedor_id = vendId || null;
          payload.quantidade = qtdNum;
          payload.qtd = qtdNum;
          payload.vl_unit = valorUnitNum;
          payload.valor_total = valorTotalCalc;
          payload.data_conclusao = conc || null;
          payload.obs = obs;
          payload.comissao_pct = Number(String(comPct).replace(',', '.')) || Number(of && of.comissao_pct || 0) || 1;
          ...
          try {
            ...
            if (r2.ok && j2 && j2.ok) {
              try { modal.style.display = 'none'; } catch (_) {}
              try { if (typeof window.toast === 'function') window.toast('OF salva com sucesso! ✅', 'var(--green)'); } catch (_) {}
              try { if (typeof window._renderComissoesPatch === 'function') window._renderComissoesPatch(); } catch (_) {}
            } else {
  ...
  function _normalizarGruposComissoes(json) {
    ...
    return grupos.map(function(g) {
      ...
      var pctGrupo = Number(g && g.comissao_pct || 0) || 1;
      var ofsNorm = ofsGrupo.map(function(of) { return _normalizarOFComissao(of, nome, pctGrupo); });
  ...
  function _renderResultadosBusca(ofs, termo) {
    var resultDiv = document.getElementById('_com_busca_resultado');
    if (!resultDiv) return;
    var todasEncontradas = Array.isArray(ofs) ? ofs : [];
    ...
    var html = '<div style="margin-top:12px">'
      + '<div style="color:#f1f5f9;font-size:15px;font-weight:700;margin-bottom:10px">Resultado da Busca</div>'
      + '<div style="color:#94a3b8;font-size:12px;margin-bottom:10px">' + String(todasEncontradas.length) + ' resultado(s) para "<b style="color:#f1f5f9">' + _escHtmlCom(termo) + '</b>"</div>'
      + '<div style="overflow:auto;border:1px solid #334155;border-radius:10px">'
      + '<table style="width:100%;border-collapse:collapse;background:#0f172a"><thead><tr style="background:#1e293b">'
      + '<th style="padding:9px 10px;text-align:left;color:#94a3b8;font-size:11px">Nº OF</th>'
      + '<th style="padding:9px 10px;text-align:left;color:#94a3b8;font-size:11px">Cliente</th>'
      + '<th style="padding:9px 10px;text-align:left;color:#94a3b8;font-size:11px">Vendedor</th>'
      + '<th style="padding:9px 10px;text-align:right;color:#94a3b8;font-size:11px">Qtd</th>'
      + '<th style="padding:9px 10px;text-align:right;color:#94a3b8;font-size:11px">Valor Total</th>'
      + '<th style="padding:9px 10px;text-align:left;color:#94a3b8;font-size:11px">Status</th>'
      + '<th style="padding:9px 10px;text-align:left;color:#94a3b8;font-size:11px">Data Conclusão</th>'
      + '<th style="padding:9px 10px;text-align:center;color:#94a3b8;font-size:11px">Ações</th>'
      + '</tr></thead><tbody>'
      + todasEncontradas.map(function(of, idx) {
        var numero = String(of && (of.numero || of.num_of || of.numero_of || of.of) || '—');
        ...
        var dataStr = '—';
        try { dataStr = of && of.data_conclusao ? new Date(of.data_conclusao).toLocaleDateString('pt-BR') : '—'; } catch (_) {}
        var id = String(of && of.id || '');
        var btnConcluir = !concluida
          ? '<button class="_btn_concluir_of" data-of-id="' + _escHtmlCom(id) + '" data-of-num="' + _escHtmlCom(numero) + '" style="background:#16a34a;color:#fff;border:none;border-radius:6px;padding:6px 12px;cursor:pointer;font-size:12px">Concluir</button>'
          : '';
        var btnEditar = '<button class="_btn_editar_of" data-of-id="' + _escHtmlCom(id) + '" style="background:#1d4ed8;color:#fff;border:none;border-radius:6px;padding:6px 12px;cursor:pointer;font-size:12px">Trocar</button>';
        return '<tr style="border-top:1px solid #1e293b;background:' + (idx % 2 ? 'rgba(255,255,255,0.02)' : 'transparent') + '">'
          + '<td style="padding:8px 10px;color:#60a5fa">#' + _escHtmlCom(numero) + '</td>'
          + '<td style="padding:8px 10px;color:#e2e8f0">' + _escHtmlCom(cliente) + '</td>'
          + '<td style="padding:8px 10px;color:#cbd5e1">' + _escHtmlCom(vendedor) + '</td>'
          + '<td style="padding:8px 10px;text-align:right;color:#cbd5e1">' + qtd.toLocaleString('pt-BR') + '</td>'
          + '<td style="padding:8px 10px;text-align:right;color:#e2e8f0">' + _escHtmlCom(_fmtMoney(valorTotal)) + '</td>'
          + '<td style="padding:8px 10px;color:#cbd5e1">' + _escHtmlCom(status) + '</td>'
          + '<td style="padding:8px 10px;color:#cbd5e1">' + _escHtmlCom(dataStr) + '</td>'
          + '<td style="padding:8px 10px;text-align:center;white-space:nowrap">' + btnConcluir + (btnConcluir ? ' ' : '') + btnEditar + '</td>'
          + '</tr>';
      }).join('')
      + '</tbody></table></div></div>';
    ...
    try {
      Array.prototype.slice.call(resultDiv.querySelectorAll('._btn_editar_of')).forEach(function(btn) {
        ...
        btn.addEventListener('click', function() {
          var ofId = '';
          try { ofId = String(this.getAttribute('data-of-id') || this.dataset.ofId || '').trim(); } catch (_) { ofId = ''; }
          if (ofId && typeof window._abrirModalEdicaoOF === 'function') window._abrirModalEdicaoOF(ofId);
        }, true);
      });
    } catch (_) {}
    try {
      window.__comBuscaDestacados = Object.create(null);
      todasEncontradas.forEach(function(of) {
        var num = String(of && (of.numero || of.of || of.of_numero) || '').trim();
        var status = String(of && of.status || '').toLowerCase();
        if (num && status.indexOf('conclu') >= 0) window.__comBuscaDestacados[num] = true;
      });
      if (typeof window._aplicarDestaqueBuscaComissoes === 'function') window._aplicarDestaqueBuscaComissoes();
    } catch (_) {}
  }
  ...
  window._buscarOFsComissao = async function(termo) {
    ...
    try {
      var termos = String(termo).split(/[,\s]+/)
        .map(function(t) { return t.trim().toLowerCase(); })
        .filter(function(t) { return t.length > 0; });
      if (!termos.length) termos = [String(termo || '').toLowerCase()];

      var token = '';
      try { token = String(localStorage.getItem('token') || localStorage.getItem('access_token') || sessionStorage.getItem('token') || '').trim(); } catch (_) { token = ''; }
      var headersBusca = token ? { Authorization: 'Bearer ' + token } : {};
      var promessas = termos.map(function(t) {
        return fetch('/api/ofs?search=' + encodeURIComponent(t) + '&limit=50', {
          headers: headersBusca
        })
          .then(function(r) { return r.ok ? r.json() : []; })
          .then(function(d) { return Array.isArray(d) ? d : (d.ofs || d.data || []); })
          .catch(function() { return []; });
      });

      Promise.all(promessas).then(function(resultados) {
        var idsVistos = new Set();
        var todasEncontradas = [];
        resultados.forEach(function(lista) {
          (lista || []).forEach(function(of) {
            var key = of && (of.id || of.numero);
            if (!idsVistos.has(key)) {
              var bate = termos.some(function(t) {
                return String(of && (of.numero || of.num_of || of.of) || '').toLowerCase().includes(t) ||
                       String(of && (of.cli_nome || of.cliente_nome || of.cliente || of.cliNome) || '').toLowerCase().includes(t);
              });
              if (bate) {
                idsVistos.add(key);
                todasEncontradas.push(of);
              }
            }
          });
        });
        _renderResultadosBusca(todasEncontradas, termo);
      }).catch(function(e) {
  ...
  async function _renderComissoesPatch() {
    ...
    try {
      ...
      var grupos = (vendedores || []).map(function(v) {
        ...
        var pctGrupo = Number(v && v.comissao_pct || 0) || 1;
        var ofsNorm = ofsVendedor.map(function(of) { return _normalizarOFComissao(of, vendNome, pctGrupo); });
        return {
          id: id,
          vendedor: vendNome,
      ...
      if (window.__comissoesPrevData && prevTotal > 0) {
        ...
        varPct = '<div style="color:' + cor + ';font-size:12px;margin-top:4px">' + seta + ' ' + Math.abs(diff).toFixed(1) + '% vs mês anterior</div>'
          + '<div style="color:#94a3b8;font-size:11px;margin-top:4px">' + _escHtmlCom(_fmtRs(prevTotal)) + ' → ' + _escHtmlCom(_fmtRs(totalGeral)) + '</div>';
      }
      ...
      var htmlCards = ''
        ...
        + '  <div style="background:#1e293b;border-radius:10px;padding:16px 20px"><div style="color:#64748b;font-size:12px;text-transform:uppercase">COMPARAÇÃO DE VENDAS</div>' + (varPct || '<div style="color:#94a3b8;font-size:12px;margin-top:6px">—</div>') + '</div>'
        + '</div>';
      ...
      htmlDetalhe += grupos.map(function(g, gi) {
        return ''
          ...
          + g.ofs.map(function(of) {
            ...
            var destaqueBusca = !!(window.__comBuscaDestacados && window.__comBuscaDestacados[String(of.numero || '').trim()]);
            return ''
              + '<tr data-of-row="1" data-search="' + _escHtmlCom(searchTxt) + '" data-of-numero="' + _escHtmlCom(of.numero) + '" data-of-cliente="' + _escHtmlCom(of.cliente) + '" data-of-status="' + _escHtmlCom(of.status) + '" data-of-id="' + _escHtmlCom(of.id) + '" style="border-bottom:1px solid #1e293b;' + (destaqueBusca ? 'background:rgba(234,179,8,.22);' : '') + '">'
      ...
      _renderizarBuscaOFs();
      window._aplicarDestaqueBuscaComissoes = function() {
        try {
          Array.prototype.slice.call(document.querySelectorAll('tr[data-of-row="1"]')).forEach(function(tr) {
            var num = String(tr.getAttribute('data-of-numero') || '').trim();
            tr.style.background = (window.__comBuscaDestacados && window.__comBuscaDestacados[num]) ? 'rgba(234,179,8,.22)' : '';
          });
        } catch (_) {}
      };
      try {
...
window._renderGramaturas = async function() {
  ...
  var ovAntigo = document.getElementById('_overlay_gramaturas');
  if (ovAntigo) ovAntigo.remove();
  
  var content = document.getElementById('content');
  if (!content) content = document.querySelector('.content, .main-area, #main-content');
  if (!content) return;
  
  var host = document.getElementById('page-gramaturas');
  if (!host) {
    host = document.createElement('div');
    host.id = 'page-gramaturas';
    content.appendChild(host);
  }
  host.style.cssText = 'display:block;padding:20px;overflow-y:auto;height:100%;box-sizing:border-box';
  host.innerHTML = '<h2 style="color:var(--text,#fff);margin-bottom:20px">📄 Gramaturas</h2>'
  ...
  try {
    ...
    var fornecedores = Object.keys(grupos).sort();
    host.innerHTML =
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">'
      + '<h2 style="color:var(--text,#fff);margin:0">📄 Gramaturas</h2>'
      + '<button onclick="window._abrirModalNovaGramatura()" '
      + 'style="background:#6366f1;color:#fff;border:none;border-radius:8px;padding:10px 20px;cursor:pointer;font-weight:600">+ Nova Gramatura</button>'
      + '</div>'
      + (fornecedores.length ? fornecedores.map(function(forn) {
        ...
        return '<details open style="margin-bottom:12px;background:rgba(255,255,255,0.03);border:1px solid var(--border,#333);border-radius:12px;overflow:hidden">'
          ...
          + '</tr></thead><tbody>'
          + itens.map(function(g, i) {
            return '<tr style="border-top:1px solid var(--border,#333);background:'
              + (i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.02)') + '">'
              + '<td style="padding:9px 12px;color:var(--text,#fff)">' + String(g.nome || '—').replace(/</g, '&lt;') + '</td>'
              + '<td style="padding:9px 12px;text-align:right;color:#6366f1;font-weight:700">' + String(g.gramatura || '—') + '</td>'
              ...
              + 'style="background:transparent;border:1px solid rgba(239,68,68,0.4);color:#ef4444;border-radius:6px;padding:4px 10px;cursor:pointer;font-size:12px">Excluir</button></td></tr>';
          }).join('')
          + '</tbody></table></details>';
      }).join('') : '<div style="padding:20px;text-align:center;color:#aaa;border:1px solid var(--border,#333);border-radius:12px">Nenhuma gramatura cadastrada</div>');
  } catch (e) {
    host.innerHTML = '<h2 style="color:#fff;margin-bottom:20px">📄 Gramaturas</h2>'
...
window._renderToneladasVendidas = async function() {
  ...
  var ovAntigo = document.getElementById('_overlay_toneladas');
  if (ovAntigo) ovAntigo.remove();
  ...
  var host = document.getElementById('page-toneladas-vendidas');
  if (!host) {
    host = document.createElement('div');
    host.id = 'page-toneladas-vendidas';
    content.appendChild(host);
  }
  host.style.cssText = 'display:block;padding:20px;overflow-y:auto;height:100%;box-sizing:border-box';
  var hoje = new Date();
  ...
  host.innerHTML =
    ...
    + '</div>'
    + '<div style="background:var(--bg2,#1a1a2e);border-radius:12px;padding:20px;border:1px solid var(--border,#333)">'
    + '<div style="font-size:12px;color:#aaa;margin-bottom:8px">CUSTO ESTIMADO PAPEL</div>'
    + '<div id="ton-card-custo" style="font-size:32px;font-weight:700;color:#22c55e">R$ 0,00</div>'
    + '</div>'
    + '<div style="background:var(--bg2,#1a1a2e);border-radius:12px;padding:20px;border:1px solid var(--border,#333)">'
...
window._recarregarToneladas = async function() {
  ...
  function fT(v) { return Number(v || 0).toFixed(3) + ' t'; }
  function fM(v) { return Number(v || 0).toFixed(2) + ' m²'; }
  function fR(v) { return 'R$ ' + Number(v || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }); }
  function renderSecao(titulo, itens, colunas) {
    return '<div style="margin-bottom:24px">'
      + '<div style="font-size:16px;font-weight:700;color:var(--text,#fff);margin-bottom:10px">' + escHtml(titulo) + '</div>'
      + (!itens.length
        ? '<div style="color:#94a3b8;padding:16px;border:1px solid var(--border,#333);border-radius:12px">Sem dados no período.</div>'
        : '<div style="overflow:auto;border-radius:12px;border:1px solid var(--border,#333)">'
          + '<table style="width:100%;border-collapse:collapse">'
          + '<thead><tr style="background:var(--bg2,#1a1a2e)">' + colunas.map(function(col) {
            return '<th style="padding:10px 12px;text-align:' + (col.align || 'left') + ';color:#aaa;font-size:11px">' + escHtml(col.label) + '</th>';
          }).join('') + '</tr></thead>'
          + '<tbody>' + itens.map(function(item, idx) {
            return '<tr style="border-top:1px solid var(--border,#333);background:' + (idx % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.02)') + '">'
              + colunas.map(function(col) {
                var raw = typeof col.render === 'function' ? col.render(item) : item[col.key];
                return '<td style="padding:9px 12px;text-align:' + (col.align || 'left') + ';color:' + (col.color || '#e2e8f0') + ';' + (col.bold ? 'font-weight:700;' : '') + '">' + raw + '</td>';
              }).join('') + '</tr>';
  ...
  try {
    ...
    if (!json || !json.ok) {
      if (corpo) corpo.innerHTML = '<div style="color:#f87171;padding:20px">Erro: ' + escHtml(json && json.error || 'falha') + '</div>';
      return;
    }
    ...
    detalhes.forEach(function(d) {
      var gramKey = String(Number(d && d.gram || 0) || 0) + 'g';
      if (!porGramaturaMap[gramKey]) porGramaturaMap[gramKey] = { gram: Number(d && d.gram || 0) || 0, area_m2: 0, toneladas: 0, ofs: 0 };
      porGramaturaMap[gramKey].area_m2 += Number(d && d.area_m2 || 0) || 0;
      porGramaturaMap[gramKey].toneladas += Number(d && d.toneladas || 0) || 0;
      porGramaturaMap[gramKey].ofs += 1;
    });
    var porClienteMap = Object.create(null);
    detalhes.forEach(function(d) {
      var cliKey = String(d && (d.cli_id || d.cliente || 'sem-cliente') || 'sem-cliente');
      if (!porClienteMap[cliKey]) porClienteMap[cliKey] = { cliente: String(d && d.cliente || '—'), area_m2: 0, toneladas: 0, ofs: 0 };
      porClienteMap[cliKey].area_m2 += Number(d && d.area_m2 || 0) || 0;
      porClienteMap[cliKey].toneladas += Number(d && d.toneladas || 0) || 0;
      porClienteMap[cliKey].ofs += 1;
    });
    var porGramatura = Object.values(porGramaturaMap).sort(function(a, b) { return Number(b.toneladas || 0) - Number(a.toneladas || 0); });
    var porCliente = Object.values(porClienteMap).sort(function(a, b) { return Number(b.toneladas || 0) - Number(a.toneladas || 0); });
    var totalTonCalc = detalhes.reduce(function(s, d) { return s + (Number(d && d.toneladas || 0) || 0); }, 0);
    var totalM2Calc = detalhes.reduce(function(s, d) { return s + (Number(d && d.area_m2 || 0) || 0); }, 0);
    var totalCustoCalc = detalhes.reduce(function(s, d) { return s + (Number(d && d.custo_estimado || 0) || 0); }, 0);
    ...
    var cardM2 = document.getElementById('ton-card-m2');
    var cardOfs = document.getElementById('ton-card-ofs');
    if (cardTon) cardTon.textContent = fT(totalTonCalc);
    if (cardM2) cardM2.textContent = fM(totalM2Calc);
    if (cardOfs) cardOfs.textContent = String(detalhes.length || 0);
    var cardCusto = document.getElementById('ton-card-custo');
    if (cardCusto) cardCusto.textContent = 'R$ ' + Number(totalCustoCalc || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
(function _injetarMenusCustom() {
  ...
  function _clonarItemMenu(pageIdRef, novoPageId, novoTexto, novoId) {
    ...
    var seletores = [
      '.sidebar [onclick*="' + pageIdRef + '"]',
      '[class*="sidebar"] [onclick*="' + pageIdRef + '"]',
      '.nav [onclick*="' + pageIdRef + '"]',
      '[class*="nav-group"] [onclick*="' + pageIdRef + '"]',
      '[onclick*="' + pageIdRef + '"]'
    ];

    var ref = null;
    for (var i = 0; i < seletores.length; i++) {
      try { ref = document.querySelector(seletores[i]); } catch (_) {}
      if (ref) break;
    }
    ...
    novo.querySelectorAll('[id]').forEach(function(el) { el.removeAttribute('id'); });

    var spanTxt = novo.querySelector('span:not([class*="ico"]):not([class*="icon"])');
    if (spanTxt) {
      spanTxt.textContent = novoTexto;
    } else {
      var textoAtualizado = false;
      novo.childNodes.forEach(function(n) {
        if (n.nodeType === 3 && n.textContent.trim().length > 1 && !textoAtualizado) {
          n.textContent = ' ' + novoTexto;
          textoAtualizado = true;
        }
      });
      if (!textoAtualizado) {
        var ultimoSpan = novo.querySelector('span');
        if (ultimoSpan) ultimoSpan.textContent = novoTexto;
      }
    }

    ref.parentElement.appendChild(novo);
    console.log('[PATCH] Menu injetado:', novoId, '- texto:', novo.textContent.trim().slice(0, 30));
    return true;
  }
  ...
  setTimeout(function() {
    ...
    itensFinanceiro.forEach(function(el) {
      var txt = String(el.textContent || '').trim();
      if (txt.indexOf('Inconformidade') >= 0 || txt.indexOf('inconformidade') >= 0) {
        el.childNodes.forEach(function(n) {
          if (n.nodeType === 3 && n.textContent.trim().length > 1) {
            n.textContent = ' Caixas Perdidas';
          }
        });
        var spans = el.querySelectorAll('span');
        spans.forEach(function(s) {
          if (s.textContent.trim().indexOf('Inconformidade') >= 0) {
            s.textContent = 'Caixas Perdidas';
          }
        });
      }
    });
