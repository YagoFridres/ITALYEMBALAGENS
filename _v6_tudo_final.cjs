...
(async () => {
  try {
    ...
    let patchV = null;
    for (let i = 0; i < 20; i++) {
      const p = await R('GET', '/api/_patch_info');
      patchV = (p.j?.ok && p.j.patch_runtime) ? p.j.patch_runtime : (p.j?.patch_runtime || null);
      if (patchV === '20260812150500') break;
      L({ t: 'WAIT_DEPLOY', i, patchV });
      await S(10000);
    }
    L({ t: 'DEPLOY_OK', patchV });
    ...
    const dels1 = [];
    for (const o of numAlvos) { const d = await R('DELETE', '/api/ofs/' + o.id); dels1.push({ id: o.id, num: o.numero, s: d.s, ok: !!d.j?.ok }); }
    ...
    // C7: nome SEM cli_id pré-set mas com clinome=MÓVEIS RIPKE e sem id
    ...
    const dels2 = [];
    for (const id of idsD) { const d = await R('DELETE', '/api/ofs/' + id); dels2.push({ id: id, s: d.s, ok: !!d.j?.ok }); }
